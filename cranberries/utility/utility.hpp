...
template <class T, class U,
          std::enable_if_t<conjunction_v<is_tuple<T>, is_tuple<U>>,
...
template <class T, class U,
          std::enable_if_t<conjunction_v<is_tuple<T>, negation<is_tuple<U>>>,
                           std::nullptr_t> = nullptr>
constexpr auto make_tuple_cat(T t, U u) {
...
template <class T, class U,
          std::enable_if_t<conjunction_v<negation<is_tuple<T>>, is_tuple<U>>,
                           std::nullptr_t> = nullptr>
constexpr auto make_tuple_cat(T t, U u) {
...
template <class T, class U,
          std::enable_if_t<
              conjunction_v<negation<is_tuple<T>>, negation<is_tuple<U>>>,
              std::nullptr_t> = nullptr>
constexpr auto make_tuple_cat(T t, U u) {
...
inline constexpr cranberries_magic::return_type<D, Types...> make_array(
    Types &&... t) {
...
struct fix_result {
  ...
  constexpr auto operator()(Args &&... args) const
      noexcept(noexcept(f_(*std::declval<fix_result const *>(),
                           std::declval<Args>()...)))
          -> decltype(f_(*std::declval<fix_result const *>(),
                         std::declval<Args>()...)) {
...
constexpr auto invoke(F &&f, ArgTypes &&... args)
    ...
    noexcept(noexcept(_detail::INVOKE(std::forward<F>(f),
                                      std::forward<ArgTypes>(args)...)))
        -> decltype(_detail::INVOKE(std::forward<F>(f),
                                    std::forward<ArgTypes>(args)...)) {
...
inline constexpr decltype(auto) apply_impl(F &&f, Tuple &&t,
  ...
  return cranberries::invoke(std::forward<F>(f),
...
inline constexpr decltype(auto) apply(F &&f, Tuple &&t) {
  return cranberries_magic::apply_impl(
      std::forward<F>(f), std::forward<Tuple>(t),
      std::make_index_sequence<std::tuple_size<std::decay_t<Tuple>>::value>{});
}

template <class Tuple, std::size_t... I>
inline constexpr decltype(auto) tuple_print_impl(std::ostream &os, Tuple &&t,
                                                 std::index_sequence<I...>) {
  return [&](auto &&head, auto &&... tail) {
    os << "(" << head;
    (void)std::initializer_list<int>{(void(os << "," << tail), 0)...};
    os << ")";
  }(std::get<I>(std::forward<Tuple>(t))...);
}
...
struct PrintLine {
  ...
  void operator()(Head &&head, Tail &&... tail) {
    ...
    (void)swallow{(void(std::cout << " " << tail), 0)...};
  }
};
...
template <
    typename Range, typename BinaryOp, typename T = element_type_of_t<Range>,
    enabler_t<conjunction_v<is_range<Range>, is_callable<BinaryOp, T, T>>> =
        nullptr>
inline void adjacent_for_each(Range &&range, BinaryOp &&f) {
  // for ADL
  using std::begin;
  using std::end;
...
struct to_decimal : size_constant<(Head - size_t('0')) *
                                      Exponent_<10, sizeof...(Digits)>::value +
                                  to_decimal<Digits...>::value> {};

template <size_t Head>
struct to_decimal<Head> : size_constant<Head - size_t('0')> {};
...
struct overload<T, U, Args...> : T, overload<U, Args...> {
  using T::operator();
...
overload<typename std::decay<Funcs>::type...> make_overload(Funcs &&... funcs) {
...
constexpr T construct_from_tuple(Tuple &&t) {
  return cranberries_magic::construct_from_tuple_impl<T>(
      std::forward<Tuple>(t),
      std::make_index_sequence<std::tuple_size<std::decay_t<Tuple>>::value>{});
}

template <class Range, class F>
auto index_for_each(Range &&range, F &&f) noexcept(
    noexcept(is_nothrow_callable_v<std::decay_t<F>(
                 size_t, typename std::decay_t<Range>::value_type)>))
    -> std::enable_if_t<is_callable_v<
        std::decay_t<F>(size_t, typename std::decay_t<Range>::value_type)>> {
  size_t idx{};
  for (auto &&e : range) f(idx++, e);
}

template <class Range, class F>
auto index_for_each(size_t origin, Range &&range, F &&f) noexcept(
    noexcept(is_nothrow_callable_v<std::decay_t<F>(
                 size_t, typename std::decay_t<Range>::value_type)>))
    -> std::enable_if_t<is_callable_v<
        std::decay_t<F>(size_t, typename std::decay_t<Range>::value_type)>> {
...
template <class F, class Tuple>
struct is_apply_callable<F, Tuple,
                         cranberries::void_t<decltype(::cranberries::apply(
