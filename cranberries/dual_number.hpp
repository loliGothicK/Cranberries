...
template <typename T>
constexpr bool is_dual_v =
    std::is_base_of<cranberries_magic::dual_number_base, T>::value;
...
static constexpr T ln2 =
    static_cast<T>(0.693147180559945309417232121458176568075500134360255254120);
...
static constexpr T ln10 =
    static_cast<T>(2.302585092994045684017991454684364207601101488628772976033);
...
class dual_number : private cranberries_magic::dual_number_base {
 public:
  using value_type = T;
  constexpr dual_number(value_type const &a, value_type b) noexcept
      : real_{a}, basis_{b} {}
  constexpr dual_number(value_type const &a) noexcept : real_{a}, basis_{} {}
  constexpr dual_number() : dual_number{T{}, T{}} {}
  constexpr dual_number(dual_number const &a)
      : dual_number{a.real_, a.basis_} {}
  ...
  friend std::ostream &operator<<(std::ostream &out,
                                  dual_number const &rhs) noexcept {
...
dual_number<L> &operator+=(dual_number<L> &lhs,
                           dual_number<R> const &rhs) noexcept {
  lhs.real() += rhs.real();
  lhs.basis() += rhs.basis();
  return lhs;
}
...
template <typename T>
auto &operator+=(dual_number<T> &lhs,
                 typename dual_number<T>::value_type const &rhs) noexcept {
...
dual_number<T> operator+(dual_number<L> const &lhs,
                         dual_number<R> const &rhs) noexcept {
  return {lhs.real() + rhs.real(), lhs.basis() + rhs.basis()};
}

template <typename T>
auto operator+(dual_number<T> const &lhs,
               typename dual_number<T>::value_type const &rhs) noexcept {
  return lhs + dual_number<T>(rhs);
}

template <typename T>
auto operator+(typename dual_number<T>::value_type const &lhs,
               dual_number<T> const &rhs) noexcept {
...
dual_number<T> operator*(dual_number<L> const &lhs,
  ...
  return {lhs.real() * rhs.real(),
          lhs.basis() * rhs.real() + lhs.real() * rhs.basis()};
}
...
template <typename T>
auto operator/(dual_number<T> const &lhs,
               typename dual_number<T>::value_type const &rhs) {
  return lhs / dual_number<T>(rhs);
}

template <typename T>
auto operator/(typename dual_number<T>::value_type const &lhs,
               dual_number<T> const &rhs) {
  return dual_number<T>(lhs) / rhs;
}
...
dual_number<T> asec(dual_number<T> const &x) {
  ...
  using std::pow;
  using std::sqrt;
  return {acos(1.0 / x.real()),
          x.basis() / sqrt(pow(x.real(), 4) - x.real() * x.real())};
}
...
dual_number<T> asech(dual_number<T> const &x) {
  ...
  using std::pow;
  using std::sqrt;
  return {acosh(1.0 / x.real()),
          -x.basis() / sqrt(pow(x.real(), 2) * (1.0 - pow(x.real(), 2)))};
}
...
dual_number<T> erf(dual_number<T> const &x) noexcept {
  ...
  using std::exp;
  return {erf(x.real()),
          x.basis() * 2 * exp(-x.real() * x.real()) * pi_root_inv<T>};
}
...
std::enable_if_t<is_dual_v<DualNumber>> operator<<(
    generate_tuple_t<remove_cvr_t<element_type_of_t<DualNumber>> &, 2> &&t,
    DualNumber &&d) noexcept {
  t = std::forward_as_tuple(d.real(), d.basis());
}

template <typename T>
void operator>>(generate_tuple_t<remove_cvr_t<T>, 2> &&t,
                dual_number<T> &d) noexcept {
  std::forward_as_tuple(d.real(), d.basis()) = t;
}

namespace literals {
inline namespace dual_number_literal {
cranberries::dual_number<double> operator"" _dual(long double x) {
  return cranberries::make_dual(static_cast<double>(x), 0.0);
}
}  // namespace dual_number_literal
}  // namespace literals
