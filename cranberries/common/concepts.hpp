...
namespace concepts {
	...
	struct concept_required_seq_impl<Seq<tPack...>, Concepts...>
	{
		using type = std::enable_if_t<
			conjunction_v<required<tPack, Concepts...>...>
			>;
	};

	template < class TypePack, class... Concepts >
	using concept_required_seq = typename concept_required_seq_impl<TypePack, Concepts...>::type;
		...
		using requires
			= std::enable_if_t<
				conjunction_v<
					...
					std::is_same<Iter&, decltype(std::declval<Iter&>() -= 2)>,
					always_t<decltype(bool(std::declval<const Iter&>() < std::declval<const Iter&>()))>,
					always_t<decltype(bool(std::declval<const Iter&>() <= std::declval<const Iter&>()))>,
					always_t<decltype(bool(std::declval<const Iter&>() > std::declval<const Iter&>()))>,
					always_t<decltype(bool(std::declval<const Iter&>() >= std::declval<const Iter&>()))>,
	...
	*/
	...
	using regular_type
		= combined_concept<
			concept_<std::is_default_constructible>,
