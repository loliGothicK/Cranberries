...
#define CRANBERRIES_CONCEPT_REQUIRE(...) \
  std::enable_if_t<bool(__VA_ARGS__), std::nullptr_t> = nullptr
...
#define CRANBERRIES_HAS_TYPE(XXX, ...)                                      \
  bool(false ? ::cranberries::make_overload(                                \
                   [](auto x)                                               \
                       -> decltype(                                         \
                           std::declval<typename decltype(x)::type::XXX>(), \
...
#define CRANBERRIES_HAS_VALUE(XXX, ...)                                     \
  bool(false ? ::cranberries::make_overload(                                \
                   [](auto x) -> decltype(decltype(x)::type::XXX,           \
                                          std::true_type{}) { return {}; }, \
                   [](...) -> std::false_type { return {}; })(              \
                   ::cranberries::nested_type_class<__VA_ARGS__>{})         \
             : ::cranberries::protean_bool{})
