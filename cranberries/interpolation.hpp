...
std::array<Type, Dim + 1> lstsq(std::array<Type, N> const &x,
                                std::array<Type, N> const &y) {
...
inline auto polynomial(Container const &a,
                       typename Container::value_type x) noexcept {
  auto w = *a.crbegin();
  for (auto &&iter = a.crbegin() + 1; iter != a.crend(); ++iter)
    w = w * x + *iter;
  return w;
}
...
inline std::array<Type, N + 1> chebyshev(
    F _f, std::tuple<Type, Type> range) noexcept {
  std::array<Type, N + 1> result{};
  // static auto T = std::array<std::array<Type, N + 1>, N + 1>{};
  auto T = std::vector<std::vector<Type>>{};
  T.reserve(N + 1);
  for (size_t i = 1; i < N + 2; ++i) T.emplace_back(std::vector<Type>(i));
