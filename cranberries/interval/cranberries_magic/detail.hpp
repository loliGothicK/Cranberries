...
struct is_available_total_order<
    L, R,
    std::enable_if_t<
        std::is_same<decltype(total_less(std::declval<const L &>(),
                                         std::declval<const R &>())),
                     bool>::value>> : std::true_type {};

template <typename L, typename R>
constexpr bool is_available_total_order_v =
    is_available_total_order<L, R>::value;
...
struct is_interval {
  static constexpr bool value =
      std::is_base_of<cranberries_magic::interval_base, T>::value;
};
