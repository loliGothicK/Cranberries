...
class Expr3 : cranberries_magic::expr_base {
 ...
 public:
  Expr3(T1 const &v1, T2 const &v2, T3 const &v3) : v1_{v1}, v2_{v2}, v3_{v3} {}
  template <typename T>
  operator interval<T>() const {
    return Func::Apply(v1_.eval(), v2_.eval(), v3_.eval());
  }
  auto eval() const { return Func::Apply(v1_.eval(), v2_.eval(), v3_.eval()); }
};
...
inline constexpr auto fma_(T1 &&v1, T2 &&v2, T3 &&v3) noexcept {
  return Expr3<T1, T2, T3, Fma>(std::forward<T1>(v1), std::forward<T2>(v2),
                                std::forward<T3>(v3));
}
