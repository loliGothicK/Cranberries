...
class interval : cranberries_magic::interval_base {
...
Three namespace are defined ( weak_ordering_policy, partial_ordering and
interval_ordering_policy ).
- end note ]
*/
...
{
using namespace cranberries::weak_ordering::policy; // valid. only
constants::one ordering_policy. y < x ; // OK! equals weal_less( y, x ) ;
}
{
using namespace cranberries::weak_ordering::policy;
using namespace cranberries::weak_ordering::policy; // invalid! Two or more
ordering_policy. x < y ; // overload conflict occur.
}
- end example ]
*/
...
inline constexpr interval<T>::interval(T const &low, T const &up)
  ...
  CRANBERRIES_INVALID_ARGUMENT_THROW_WITH_MSG_IF(
      up < low, "upper_bound less than lower_bound!");
}
...
template <typename T>
inline constexpr interval<T> &interval<T>::operator=(
    interval const &x) noexcept {
  lower_ = x.lower_;
  upper_ = x.upper_;
  return *this;
}
...
/*
[ Example : You can switch high accuracy arithmetic into nomal arithmetic using
namespace inside a scope block. auto x = hull( 1.0L, 2.0L ) ; auto y = hull(
-1.0L, 1.0L ) ;
{
