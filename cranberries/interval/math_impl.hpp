...
template <typename T>
constexpr T PI() {
  return static_cast<T>(3.141592653589793238462643383279502884197L);
}
...
inline T pow_up_large(T x, size_t n) noexcept {
  ...
  UPWARD_POLICY;
  return n % 2 ? x * sq_up(pow_up_large(x, (n - 1) / 2))
               : sq_up(pow_up_large(x, n / 2));
}
...
inline T pow_down_laege(T x, size_t n) noexcept {
  ...
  DOWNWARD_POLICY;
  return n % 2 ? x * sq_down(pow_down_laege(x, (n - 1) / 2))
               : sq_down(pow_down_laege(x, n / 2));
}
template <typename T>
...
inline std::remove_reference_t<T> pow_up(T &&x, std::size_t n) noexcept {
  return n > 25 ? pow_up_large(std::forward<T>(x), n)
                : pow_up_large(std::forward<T>(x), n);
}

template <typename T>
inline std::remove_reference_t<T> pow_down(T &&x, std::size_t n) noexcept {
  return n > 25 ? pow_down_small(std::forward<T>(x), n)
                : pow_down_small(std::forward<T>(x), n);
}
...
inline std::remove_reference_t<T> sin_up(T &&arg) noexcept {
  ...
  for (auto &&e :
       {2.81145725434552E-15L, -7.647163731819816E-13L, 1.605904383682161E-10L,
        -2.505210838544172E-8L, 2.755731922398589E-6L, -1.984126984126984E-4L,
        8.33333333333333E-3L, -1.666666666666667E-1L, 1.L}) {
    w = e + w * x * x;
  }
  return sign ? -w * x : w * x;
}
...
inline std::remove_reference_t<T> cos_up(T &&arg) noexcept {
  ...
  for (auto &&e : {-1.561920696858623E-16L, 4.779477332387385E-14L,
                   -1.147074559772973E-11L, 2.08767569878681E-9L,
                   -2.755731922398589E-7L, 2.48015873015873E-5L,
                   -1.388888888888889E-3L, 4.166666666666666E-2L, -0.5L, 1.L}) {
    w = e + w * x * x;
  }
  return sign ? -w : w;
}
