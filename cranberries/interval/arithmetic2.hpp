...
inline constexpr interval<T> tan(interval<T> const &x) {
  ...
  return CRANBERRIES_OVERFLOW_ERROR_THROW_CONDITIONAL(b - a >=
                                                      interval_constants::pi<T>)
      : CRANBERRIES_OVERFLOW_ERROR_THROW_CONDITIONAL(
            static_cast<int>(b * 2 / interval_constants::pi<T>) -
                static_cast<int>(a * 2 / interval_constants::pi<T>) !=
            0)
      : CRANBERRIES_OVERFLOW_ERROR_THROW_CONDITIONAL(
            cranberries_magic::is_overflow(sin_down(a) / cos_up(a),
                                           sin_up(b) / cos_down(b)))
      : make_interval([&] { return sin_down(a) / cos_up(a); },
                      [&] { return sin_up(b) / cos_down(b); });
}
...
inline constexpr interval<T> acos(interval<T> const &x) {
  ...
  return CRANBERRIES_DOMAIN_ERROR_THROW_CONDITIONAL(x.lower() < -1.0 ||
                                                    1.0 < x.upper())
      : make_interval([&] { return acos(x.lower()); },
                      [&] { return acos(x.upper()); });
}
...
inline constexpr interval<T> asin(interval<T> const &x) {
  ...
  return CRANBERRIES_DOMAIN_ERROR_THROW_CONDITIONAL(x.lower() < -1.0 ||
                                                    1.0 < x.upper())
      : make_interval([&] { return asin(x.lower()); },
                      [&] { return asin(x.upper()); });
}
...
inline constexpr interval<T> atan(interval<T> const &x) {
  return make_interval([&] { return std::atan(x.lower()); },
                       [&] { return std::atan(x.upper()); });
}

template <typename T>
interval<T> asec(interval<T> const &x) {
  using std::acos;
  return CRANBERRIES_DOMAIN_ERROR_THROW_CONDITIONAL(
      !(x.upper() < -1.0 || 1.0 < x.lower()))
      : make_interval([&] { return acos(1.0 / x.lower()); },
                      [&] { return acos(1.0 / x.upper()); });
}
...
interval<T> acsc(interval<T> const &x) {
  ...
  return CRANBERRIES_DOMAIN_ERROR_THROW_CONDITIONAL(
      !(x.upper() < -1.0 || 1.0 < x.lower()))
      : make_interval([&] { return asin(1.0 / x.upper()); },
                      [&] { return asin(1.0 / x.lower()); });
}
...
interval<T> acot(interval<T> const &x) {
  ...
  return CRANBERRIES_OVERFLOW_ERROR_THROW_CONDITIONAL(
             x.lower() == interval_constants::zero<T> ||
             x.upper() == interval_constants::zero<T>)
      : (abs(x.lower()) < abs(x.upper()))
        ? make_interval([&] { return atan(1.0 / x.upper()); },
                        [&] { return atan(1.0 / x.lower()); })
        : make_interval([&] { return atan(1.0 / x.lower()); },
                        [&] { return atan(1.0 / x.upper()); });
}
...
inline constexpr interval<T> cosh(interval<T> const &x) {
  ...
  auto a = x.lower();
  auto b = x.upper();
  return (b < interval_constants::zero<T>)
             ? make_interval([&] { return cosh(b); }, [&] { return cosh(a); })
             : (interval_constants::zero<T> <= a)
                   ? make_interval([&] { return cosh(a); },
                                   [&] { return cosh(b); })
                   : [&] {
                       UPWARD_POLICY;
                       auto l = cosh(a);
                       auto r = cosh(b);
                       return make_interval([] { return 1.0; },
                                            [&] { return std::fmax(l, r); });
                     }();
}
...
inline constexpr interval<T> sinh(interval<T> const &x) {
  using std::sinh;
  return make_interval([&] { return sinh(x.lower()); },
                       [&] { return sinh(x.upper()); });
}
...
inline constexpr interval<T> tanh(interval<T> const &x) noexcept {
  using std::tanh;
  return make_interval([&] { return tanh(x.lower()); },
                       [&] { return tanh(x.upper()); });
}
...
inline constexpr interval<T> acosh(interval<T> const &x) {
  using std::acosh;
  return CRANBERRIES_DOMAIN_ERROR_THROW_CONDITIONAL(x.lower() < 1.0)
      : make_interval([&] { return acosh(x.lower()); },
                      [&] { return acosh(x.upper()); });
}
...
inline constexpr interval<T> asinh(interval<T> const &x) noexcept {
  using std::asinh;
  return make_interval([&] { return asinh(x.lower()); },
                       [&] { return asinh(x.upper()); });
}
...
inline interval<T> atanh(interval<T> const &x) {
  ...
  return CRANBERRIES_DOMAIN_ERROR_THROW_CONDITIONAL(x.lower() < -1.0 ||
                                                    1.0 < x.upper())
      : make_interval([&] { return atanh(x.lower()); },
                      [&] { return atanh(x.upper()); });
}

template <typename T>
inline interval<T> asech(interval<T> const &x) {
  return CRANBERRIES_DOMAIN_ERROR_THROW_CONDITIONAL(
      !(0.0 < x.lower() && x.upper() <= 1.0))
      : acosh(x.inverse());
}
...
inline constexpr interval<T> sqrt(interval<T> const &x) {
  using std::sqrt;
  return CRANBERRIES_DOMAIN_ERROR_THROW_CONDITIONAL(x.lower() <
                                                    interval_constants::zero<T>)
      : make_interval([&] { return sqrt(x.lower()); },
                      [&] { return sqrt(x.upper()); });
}
...
inline constexpr interval<T> cbrt(interval<T> const &x) noexcept {
  using std::cbrt;
  return make_interval([&] { return cbrt(x.lower()); },
                       [&] { return cbrt(x.upper()); });
}
...
inline constexpr interval<T> exp(interval<T> const &x) {
  using std::exp;
  return CRANBERRIES_OVERFLOW_ERROR_THROW_CONDITIONAL(
      cranberries_magic::is_overflow(exp(x.lower()), exp(x.upper())))
      : make_interval([&] { return exp(x.lower()); },
                      [&] { return exp(x.upper()); });
}
...
inline constexpr interval<T> exp2(interval<T> const &x) {
  using std::exp2;
  return CRANBERRIES_OVERFLOW_ERROR_THROW_CONDITIONAL(
      cranberries_magic::is_overflow(exp2(x.lower()), exp2(x.upper())))
      : make_interval([&] { return exp2(x.lower()); },
                      [&] { return exp2(x.upper()); });
}
...
inline constexpr interval<T> expm1(interval<T> const &x) {
  ...
  return cranberries_magic::is_overflow(expm1(x.lower()), expm1(x.upper()))
             ...
             : make_interval([&] { return expm1(x.lower()); },
                             [&] { return expm1(x.upper()); });
}
...
inline constexpr interval<T> log(interval<T> const &x) {
  ...
  return interval_constants::zero<T> < x.lower()
             ? make_interval([&] { return log(x.lower()); },
                             [&] { return log(x.upper()); })
...
inline constexpr interval<T> log1p(interval<T> const &x) {
  using std::log1p;
  return -1.0 < x.lower() ? make_interval([&] { return log1p(x.lower()); },
                                          [&] { return log1p(x.upper()); })
                          : CRANBERRIES_DOMAIN_ERROR_THROW_WITH_MSG(
                                "lower_bound must be greater than zero.");
}
...
inline constexpr interval<T> log10(interval<T> const &x) {
  using std::log10;
  return interval_constants::zero<T> < x.lower()
             ? make_interval([&] { return log10(x.lower()); },
                             [&] { return log10(x.upper()); })
             : CRANBERRIES_DOMAIN_ERROR_THROW_WITH_MSG(
                   "lower_bound must be greater than zero.");
}
...
inline constexpr interval<T> log2(interval<T> const &x) {
  using std::log2;
  return interval_constants::zero<T> < x.lower()
             ? make_interval([&] { return log2(x.lower()); },
                             [&] { return log2(x.upper()); })
             : CRANBERRIES_DOMAIN_ERROR_THROW_WITH_MSG(
                   "lower_bound must be greater than zero.");
}
...
inline constexpr interval<T> abs(interval<T> const &x) {
  ...
  return low < -interval_constants::zero<T> && interval_constants::zero<T> <= up
             ? make_interval([] { return interval_constants::zero<T>; },
                             [&] { return std::fmax(abs(low), abs(up)); })
             : up < interval_constants::zero<T>
                   ? make_interval([&] { return abs(up); },
                                   [&] { return abs(low); })
                   : interval<T>{x};
}

/*  interval fused multiply-add function fma( interval<T>, interval<T>,
 * interval<T>)  */
template <typename T1, typename T2, typename T3,
          typename T = std::common_type_t<T1, std::common_type_t<T2, T3>>>
inline constexpr interval<T> fma(interval<T1> const &x, interval<T2> const &y,
                                 interval<T3> const &z) noexcept {
  using std::fma;
  auto x_low = x.lower();
  auto y_low = y.lower();
  auto z_low = z.lower();
  ...
  return x_low >= interval_constants::zero<T1> &&
                 y_low >= interval_constants::zero<T2>
             ? make_interval([&] { return fma(x_low, y_low, z_low); },
                             [&] { return fma(x_up, y_up, z_up); })
             : x_low >= interval_constants::zero<T1> &&
                       y_low < interval_constants::zero<T2> &&
                       y_up > interval_constants::zero<T2>
                   ? make_interval([&] { return fma(x_up, y_low, z_low); },
                                   [&] { return fma(x_low, y_up, z_up); })
                   : x_low >= interval_constants::zero<T1> &&
                             y_up <= interval_constants::zero<T2>
                         ? make_interval(
                               [&] { return fma(x_up, y_low, z_low); },
                               [&] { return fma(x_low, y_up, z_up); })
                         : x_low < interval_constants::zero<T1> &&
                                   x_up > interval_constants::zero<T1> &&
                                   y_low >= interval_constants::zero<T2>
                               ? make_interval(
                                     [&] { return fma(x_low, y_up, z_low); },
                                     [&] { return fma(x_up, y_up, z_up); })
                               : x_low < interval_constants::zero<T1> &&
                                         x_up > interval_constants::zero<T1> &&
                                         y_low <= interval_constants::zero<T2>
                                     ? make_interval(
                                           ...
                                           [&] {
                                             return fma(x_low, y_low, z_up);
                                           })
                                     : x_up <= interval_constants::zero<T1> &&
                                               y_low >=
                                                   interval_constants::zero<T2>
                                           ? make_interval(
                                                 [&] {
                                                   return fma(x_low, y_up,
                                                              z_low);
                                                 },
                                                 [&] {
                                                   return fma(x_up, y_low,
                                                              z_up);
                                                 })
                                           : x_up <= interval_constants::zero<
                                                         T> &&
                                                     y_low <
                                                         interval_constants::
                                                             zero<T> &&
                                                     y_up > interval_constants::
                                                                zero<T>
                                                 ? make_interval(
                                                       [&] {
                                                         return fma(x_low, y_up,
...
inline constexpr interval<T> fma(interval<T1> const &x, interval<T2> const &y,
  ...
  return x_low >= interval_constants::zero<T1> &&
                 y_low >= interval_constants::zero<T2>
             ? make_interval([&] { return fma(x_low, y_low, z); },
                             [&] { return fma(x_up, y_up, z); })
             : x_low >= interval_constants::zero<T1> &&
                       y_low < interval_constants::zero<T2> &&
                       y_up > interval_constants::zero<T2>
                   ? make_interval([&] { return fma(x_up, y_low, z); },
                                   [&] { return fma(x_low, y_up, z); })
                   : x_low >= interval_constants::zero<T1> &&
                             y_up <= interval_constants::zero<T2>
                         ? make_interval([&] { return fma(x_up, y_low, z); },
                                         [&] { return fma(x_low, y_up, z); })
                         : x_low < interval_constants::zero<T1> &&
                               ...
                               : x_low < interval_constants::zero<T1> &&
                                         ...
                                         y_low <= interval_constants::zero<T2>
                                     ? make_interval(
                                           [&] { return fma(x_up, y_low, z); },
                                           [&] { return fma(x_low, y_low, z); })
                                     : x_up <= interval_constants::zero<T1> &&
                                               y_low >=
                                                   interval_constants::zero<T2>
                                           ? make_interval(
...
inline constexpr interval<T> fma(interval<T1> const &x, T const &y,
                                 interval<T3> const &z) noexcept {
  using std::fma;
  return y < interval_constants::zero<T>
             ? make_interval([&] { return fma(x.upper(), y, z.lower()); },
                             [&] { return fma(x.lower(), y, z.upper()); })
             : make_interval([&] { return fma(x.lower(), y, z.lower()); },
                             [&] { return fma(x.upper(), y, z.upper()); });
}
...
inline constexpr interval<T> fma(T const &x, interval<T2> const &y,
                                 interval<T3> const &z) noexcept {
  using std::fma;
  return x < interval_constants::zero<T>
             ? make_interval([&] { return fma(x, y.upper(), z.lower()); },
                             [&] { return fma(x, y.lower(), z.upper()); })
             : make_interval([&] { return fma(x, y.lower(), z.lower()); },
                             [&] { return fma(x, y.upper(), z.upper()); });
}

/*  interval fused multiplyply-add function fma( interval<T>, T, T)  */

template <typename T>
inline constexpr interval<T> fma(
    interval<T> const &x, typename interval<T>::value_type const &y,
    typename interval<T>::value_type const &z) noexcept {
  using std::fma;
  return y < interval_constants::zero<T>
             ? make_interval([&] { return fma(x.upper(), y, z); },
                             [&] { return fma(x.lower(), y, z); })
             : make_interval([&] { return fma(x.lower(), y, z); },
                             [&] { return fma(x.upper(), y, z); });
}
...
template <typename T>
inline constexpr interval<T> fma(
    typename interval<T>::value_type const &x, interval<T> const &y,
    typename interval<T>::value_type const &z) noexcept {
  using std::fma;
  return x < interval_constants::zero<T>
             ? make_interval([&] { return fma(x, y.upper(), z); },
                             [&] { return fma(x, y.lower(), z); })
...
template <typename T>
inline constexpr interval<T> fma(typename interval<T>::value_type const &x,
                                 typename interval<T>::value_type const &y,
                                 interval<T> const &z) noexcept {
  using std::fma;
  return make_interval([&] { return fma(x, y, z.lower()); },
                       [&] { return fma(x, y, z.upper()); });
}
...
inline constexpr interval<T> erf(interval<T> const &x) noexcept {
  using std::erf;
  return make_interval([&] { return erf(x.lower()); },
                       [&] { return erf(x.upper()); });
}
...
inline constexpr interval<T> erfc(interval<T> const &x) noexcept {
  using std::erfc;
  return make_interval([&] { return erfc(x.upper()); },
                       [&] { return erfc(x.lower()); });
}
