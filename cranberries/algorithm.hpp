...
week_day zeller(std::string yyyymmdd) {
  std::regex re{"/|-"};
  auto iter =
      std::sregex_token_iterator{yyyymmdd.begin(), yyyymmdd.end(), re, -1};
...
template <class BidirectionalIterator>
inline constexpr bool prev_combination(BidirectionalIterator first,
                                       BidirectionalIterator middle,
                                       BidirectionalIterator last) {
...
template <class BidirectionalIterator, class T>
inline constexpr bool next_mapping(BidirectionalIterator first,
                                   BidirectionalIterator last, T first_value,
                                   T last_value) {
  if (last == first) {
    return false;
  }
...
inline constexpr bool next_mapping(BidirectionalIterator first,
                                   BidirectionalIterator last, T first_value,
                                   T last_value, Incrementer increment) {
  if (last == first) {
    return false;
  }
  do {
...
template <class BidirectionalIterator, class T>
inline constexpr bool prev_mapping(BidirectionalIterator first,
                                   BidirectionalIterator last, T first_value,
                                   T last_value) {
  if (last == first) {
    return false;
...
inline constexpr bool prev_mapping(BidirectionalIterator first,
                                   BidirectionalIterator last, T first_value,
                                   T last_value, Decrementer decrement) {
  if (last == first) {
    return false;
  }
...
template <class BidirectionalIterator>
inline constexpr bool next_combination_counts(BidirectionalIterator first,
                                              BidirectionalIterator last) {
  BidirectionalIterator current = last;
  while (current != first && *(--current) == 0) {
  }
  if (current == first) {
    if (first != last && *first != 0) std::iter_swap(--last, first);
    return false;
  }
  --(*current);
...
template <class BidirectionalIterator>
inline constexpr bool prev_combination_counts(BidirectionalIterator first,
                                              BidirectionalIterator last) {
...
class default_get_key {
 ...
 public:
  ...
  auto operator()(T const &a) const noexcept {
    return endian::native == endian::little ? byte_swap(to_radix_key<T>{a}.key)
...
class reverse_key {
 ...
 public:
  ...
  T operator()(cranberries::int_fast_t<sizeof(T) * 8> const &a) const noexcept {
    return endian::native == endian::little ? (to_radix_key{byte_swap(a)}.value)
...
class radix_key<sort_type::ascending> {
 ...
 public:
  constexpr radix_key(size_t bits) noexcept : BITS{bits}, bit{} {}
...
radix_key<sort_type::ascending> make_ascending_radix_key(size_t bits,
                                                         size_t e = 0) {
  return {bits, e};
}
radix_key<sort_type::descending> make_descending_radix_key(size_t bits,
                                                           size_t e = 0) {
  return {bits, e};
}
template <class F = default_get_key>
radix_key<sort_type::ascending, F> make_ascending_radix_get_key(
    size_t bits, size_t e = 0, F f = default_get_key{}) {
  return {bits, e, f};
}
template <class F = default_get_key>
radix_key<sort_type::descending, F> make_descending_radix_get_key(
    size_t bits, size_t e = 0, F f = default_get_key{}) {
  return {bits, e, f};
}
...
template <class ForwardIterator,
          enabler_t<std::is_integral<std::decay_t<
              decltype(*std::declval<ForwardIterator>())>>::value> = nullptr>
inline void ascending_radix_sort(ForwardIterator first, ForwardIterator last) {
  size_t BIT =
      sizeof(std::decay_t<decltype(*std::declval<ForwardIterator>())>) * 8;
...
inline void descending_radix_sort(ForwardIterator first, ForwardIterator last) {
  ...
  // partition negative number to left
  auto mid =
      std::stable_partition(first, last, [](auto v) { return !(v < 0); });
...
template <class ForwardIterator, class F,
          enabler_t<std::is_integral<std::decay_t<decltype(std::declval<F>()(
              *std::declval<ForwardIterator>()))>>::value> = nullptr>
inline void ascending_radix_sort(ForwardIterator first, ForwardIterator last,
                                 F &&get_key) {
  size_t BIT =
      sizeof(decltype(std::declval<F>()(*std::declval<ForwardIterator>()))) * 8;
...
template <class ForwardIterator,
          enabler_t<!std::is_integral<std::decay_t<
              decltype(*std::declval<ForwardIterator>())>>::value> = nullptr>
void descending_radix_sort(ForwardIterator first,
                           ForwardIterator last) = delete;
