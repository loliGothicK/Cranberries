...
class lap_type {
 ...
 public:
  ...
  operator size_t() const { return duration.count(); }
...
class TimerBaseChrono {
 ...
 public:
  ...
  tick_t get_ticks() const noexcept {
    return is_started() ? _clock::now() : tick_t{};
  }
  ...
  lap_t get_lap(tick_t last_lap) const noexcept {
    return is_started() ? lap_t(std::chrono::duration_cast<Resolution>(
                              last_lap - start_))
                        : lap_t{};
  }
  ...
  lap_t duration(tick_t begin_, tick_t last_) {
    return lap_t{std::chrono::duration_cast<Resolution>(last_ - begin_)};
  }
