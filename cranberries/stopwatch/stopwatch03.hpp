...
struct hours {
  static std::size_t count(tick_counter::tick_type b,
                           tick_counter::tick_type e) {
    return tick_counter::to_hours(b, e);
  }
};
struct minutes {
  static std::size_t count(tick_counter::tick_type b,
                           tick_counter::tick_type e) {
    return tick_counter::to_min(b, e);
  }
};
struct seconds {
  static std::size_t count(tick_counter::tick_type b,
                           tick_counter::tick_type e) {
    return tick_counter::to_sec(b, e);
  }
};
struct milliseconds {
  static std::size_t count(tick_counter::tick_type b,
                           tick_counter::tick_type e) {
    return tick_counter::to_milli(b, e);
  }
};
struct microseconds {
  static std::size_t count(tick_counter::tick_type b,
                           tick_counter::tick_type e) {
    return tick_counter::to_micro(b, e);
  }
};
...
class lap_type {
 ...
 public:
  explicit lap_type(tick_counter::tick_type b, tick_counter::tick_type e)
      : begin_(b), end_(e) {}
  lap_type() : begin_(), end_() {}

  operator std::size_t() const { return Resolution::count(begin_, end_); }

  friend std::ostream &operator<<(std::ostream &os, const lap_type &lap) {
    return os << Resolution::count(lap.begin_, lap.end_)
...
class TimerBaseClock {
 public:
  ...
  lap_t get_lap(tick_t dt) throw() { return lap_t(start_, dt); }
