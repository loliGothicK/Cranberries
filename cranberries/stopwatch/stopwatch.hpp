...
namespace cranberries {
  ...
  */
  ...
  public:
    ...
    explicit basic_stopwatch( char const* activity = "Stopwatch", timer::config start = timer::config::auto_ ) CRANBERRIES_NOEXCEPT;
    basic_stopwatch( std::ostream& log, char const* activity = "Stopwatch", timer::config start = timer::config::auto_ ) CRANBERRIES_NOEXCEPT;
  ...
  template <typename T> inline basic_stopwatch<T>::basic_stopwatch(timer::config start_now ) CRANBERRIES_NOEXCEPT
		: mode_( start_now )
    , activity_( "Stopwatch" )
    , lap_()
    , log_( std::cout )
  ...
  template <typename T> inline basic_stopwatch<T>::basic_stopwatch( char const* activity, timer::config start_now )  CRANBERRIES_NOEXCEPT
		: mode_( start_now )
    ...
    , log_( std::cout )
		...
		, elapsed_()
	{
		if (static_cast<bool>(start_now)) activity_ ? start() : start( CRANBERRIES_NULLPTR );
  }

  //  set log output, optional printout, optional start
  template <typename T> inline basic_stopwatch<T>::basic_stopwatch( std::ostream& log, char const* activity, timer::config start_now )  CRANBERRIES_NOEXCEPT
    : mode_( start_now )
		, activity_( activity && activity[0] ? activity : CRANBERRIES_NULLPTR )
    , lap_()
    , log_( log )
  ...
  template <typename T> inline typename basic_stopwatch<T>::lap_t basic_stopwatch<T>::lap() CRANBERRIES_NOEXCEPT {
		if (is_started()) {
  ...
  template <typename T> inline typename basic_stopwatch<T>::tick_t basic_stopwatch<T>::show( char const* event_name ) CRANBERRIES_NOEXCEPT {
    return is_started() ? (( lap_ = BaseTimer::get_ticks()
      ,( (event_name && event_name[0] ? log_ << activity_ << ": " : log_ ) << event_name << " at " << BaseTimer::get_lap( lap_ ) << "[ms]" << std::endl))
      , lap_)
      : activity_ ? ( log_ << activity_ << ": not started" << std::endl, lap_ ) : lap_;
  }

  //   (re)start a stopwatch, set/return lap time
  template <typename T> inline typename basic_stopwatch<T>::tick_t basic_stopwatch<T>::start() CRANBERRIES_NOEXCEPT {
    if (is_started()) {
      stop();
    }
		laps_.clear();
  ...
  template <typename T>
  inline typename basic_stopwatch<T>::tick_t basic_stopwatch<T>::stop() CRANBERRIES_NOEXCEPT {
		if (is_started()) {
			lap_ = BaseTimer::get_ticks();
			...
			if (!laps_.empty()) {
				...
				for (size_t i = 1; i < laps_.size(); ++i) {
					log_ << "\t\t{\n\t\t\ttime"
						...
						<< ",\n\t\t\tinterval: " << BaseTimer::duration(laps_[i - 1], laps_[i]) << "\n\t\t},\n";
				}
				log_ << "\t],\n";
			}
	...
	template <typename F>
	basic_stopwatch<T>&
	basic_stopwatch<T>::elapsed(F target) {
