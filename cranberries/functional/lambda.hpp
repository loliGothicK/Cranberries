...
template <class T>
struct is_expr<T, cranberries::void_t<typename remove_cvr_t<T>::is_expression>>
    : std::true_type {};
...
template <class T>
using is_expression =
    cranberries::disjunction<std::is_bind_expression<T>, is_expr<T>,
                             cranberries::is_bind_expression<T>>;
...
struct ValWrapper_<placeholder_t<N>> {
  ...
  constexpr decltype(auto) eval(Tuple &&t) const noexcept {
...
struct expr_bind {
  using type =
      std::conditional_t<is_expr<T>::value, T, ValWrapper_<std::decay_t<T>>>;
};
...
template <class T>
struct _index<T, std::enable_if_t<is_expr_v<remove_cvr_t<T>> &&
                                  !is_placeholder_v<remove_cvr_t<T>>>>
    : size_constant<remove_cvr_t<T>::value> {};

template <class T>
constexpr size_t _index_v = _index<T>::value;

template <class Func, class... Args>
class LambdaExpression : public pack_max<size_pack<_index_v<Args>...>> {
  template <class Accept, class Tuple, size_t... I>
  static constexpr auto calc(Accept &&ac, Tuple &&t,
                             std::index_sequence<I...>) {
    return Func::apply((
        std::get<I>(std::forward<Tuple>(t)).eval(std::forward<Accept>(ac)))...);
  }
 ...
 public:
  ...
  constexpr auto eval(AcceptArgs &&ac) const {
    return calc(std::forward<AcceptArgs>(ac), args,
                std::index_sequence_for<Args...>{});
  }

  template <class... AcceptArgs>
  constexpr auto operator()(AcceptArgs &&... ac) const {
    return calc(std::forward_as_tuple(std::forward<AcceptArgs>(ac)...), args,
                std::index_sequence_for<Args...>{});
  }
};
...
struct Fma_ {
  ...
  static constexpr auto apply(T1 &&v1, T2 &&v2, T3 &&v3) noexcept {
    return fma(std::forward<T1>(v1), std::forward<T2>(v2),
               std::forward<T3>(v3));
  }
};
...
constexpr decltype(auto) _check(L &&l, const placeholder_t<0> &) {
  return std::make_tuple(std::forward<L>(l),
                         placeholder_t<remove_cvr_t<L>::value + 1>{});
}
...
constexpr decltype(auto) _check(const placeholder_t<0> &, R &&r) {
  return std::make_tuple(placeholder_t<remove_cvr_t<R>::value + 1>{},
                         std::forward<R>(r));
}
...
template <class L, class R,
          enabler_t<!std::is_same<remove_cvr_t<L>, placeholder_t<0>>::value> =
              nullptr,
          enabler_t<!std::is_same<remove_cvr_t<R>, placeholder_t<0>>::value> =
              nullptr>
...
template <class Op, class Arg,
          enabler_t<!is_tuple_v<remove_cvr_t<Arg>>> = nullptr>
...
template <class Op, class Tuple,
          enabler_t<is_tuple_v<remove_cvr_t<Tuple>>> = nullptr>
...
constexpr auto lt_(L &&lhs, R &&rhs) noexcept {
  return construct_lambda<lambda_operations::LT_>(
      _check(std::forward<L>(lhs), std::forward<R>(rhs)));
}

template <class L, class R>
constexpr auto le_(L &&lhs, R &&rhs) noexcept {
  return construct_lambda<lambda_operations::LE_>(
      _check(std::forward<L>(lhs), std::forward<R>(rhs)));
}

template <class L, class R>
constexpr auto gt_(L &&lhs, R &&rhs) noexcept {
  return construct_lambda<lambda_operations::GT_>(
      _check(std::forward<L>(lhs), std::forward<R>(rhs)));
}

template <class L, class R>
constexpr auto ge_(L &&lhs, R &&rhs) noexcept {
  return construct_lambda<lambda_operations::GE_>(
      _check(std::forward<L>(lhs), std::forward<R>(rhs)));
}

template <class L, class R>
constexpr auto eq_(L &&lhs, R &&rhs) noexcept {
  return construct_lambda<lambda_operations::EQ_>(
      _check(std::forward<L>(lhs), std::forward<R>(rhs)));
}

template <class L, class R>
constexpr auto ne_(L &&lhs, R &&rhs) noexcept {
  return construct_lambda<lambda_operations::NE_>(
      _check(std::forward<L>(lhs), std::forward<R>(rhs)));
}

template <class L, class R>
constexpr auto bitand_(L &&lhs, R &&rhs) noexcept {
  return construct_lambda<lambda_operations::BitAnd_>(
      _check(std::forward<L>(lhs), std::forward<R>(rhs)));
}

template <class L, class R>
constexpr auto xor_(L &&lhs, R &&rhs) noexcept {
  return construct_lambda<lambda_operations::Xor_>(
      _check(std::forward<L>(lhs), std::forward<R>(rhs)));
}

template <class L, class R>
constexpr auto bitor_(L &&lhs, R &&rhs) noexcept {
  return construct_lambda<lambda_operations::BitOr_>(
      _check(std::forward<L>(lhs), std::forward<R>(rhs)));
}

template <class L, class R>
constexpr auto and_(L &&lhs, R &&rhs) noexcept {
  return construct_lambda<lambda_operations::LogicalAnd_>(
      _check(std::forward<L>(lhs), std::forward<R>(rhs)));
}

template <class L, class R>
constexpr auto or_(L &&lhs, R &&rhs) noexcept {
  return construct_lambda<lambda_operations::LogicalOr_>(
      _check(std::forward<L>(lhs), std::forward<R>(rhs)));
}

template <class L, class R>
constexpr auto add_(L &&lhs, R &&rhs) noexcept {
  return construct_lambda<lambda_operations::Add_>(
      _check(std::forward<L>(lhs), std::forward<R>(rhs)));
}
template <class L, class R>
constexpr auto sub_(L &&lhs, R &&rhs) noexcept {
  return construct_lambda<lambda_operations::Sub_>(
      _check(std::forward<L>(lhs), std::forward<R>(rhs)));
}

template <class L, class R>
constexpr auto mul_(L &&lhs, R &&rhs) noexcept {
  return construct_lambda<lambda_operations::Mul_>(
      _check(std::forward<L>(lhs), std::forward<R>(rhs)));
}

template <class L, class R>
constexpr auto div_(L &&lhs, R &&rhs) noexcept {
  return construct_lambda<lambda_operations::Div_>(
      _check(std::forward<L>(lhs), std::forward<R>(rhs)));
}
...
constexpr auto asinh_(A &&a) noexcept {
  return construct_lambda<lambda_operations::Asinh_>(
      _check(std::forward<A>(a)));
}

template <class A>
constexpr auto acosh_(A &&a) noexcept {
  return construct_lambda<lambda_operations::Acosh_>(
      _check(std::forward<A>(a)));
}

template <class A>
constexpr auto atanh_(A &&a) noexcept {
  return construct_lambda<lambda_operations::Atanh_>(
      _check(std::forward<A>(a)));
}
...
constexpr auto expm1_(A &&a) noexcept {
  return construct_lambda<lambda_operations::Expm1_>(
      _check(std::forward<A>(a)));
}
...
constexpr auto log10_(A &&a) noexcept {
  return construct_lambda<lambda_operations::Log10_>(
      _check(std::forward<A>(a)));
}
...
constexpr auto log1p_(A &&a) noexcept {
  return construct_lambda<lambda_operations::Log1p_>(
      _check(std::forward<A>(a)));
}
...
constexpr auto pow_(L &&a, R &&n) noexcept {
  return construct_lambda<lambda_operations::Pow_>(
      _check(std::forward<L>(a), std::forward<R>(n)));
}

template <class T1, class T2, class T3>
constexpr auto fma_(T1 &&v1, T2 &&v2, T3 &&v3) noexcept {
  return construct_lambda<lambda_operations::Fma_>(
      _check(std::forward<T1>(v1), std::forward<T2>(v2), std::forward<T3>(v3)));
}

template <typename L, typename R>
constexpr auto max_(L &&v1, R &&v2) noexcept {
  return construct_lambda<lambda_operations::Max_>(
      _check(std::forward<L>(v1), std::forward<R>(v2)));
}

template <typename L, typename R>
constexpr auto min_(L &&v1, R &&v2) noexcept {
  return construct_lambda<lambda_operations::Min_>(
      _check(std::forward<L>(v1), std::forward<R>(v2)));
}
...
template <typename... Ts>
constexpr LambdaExpression<lambda_operations::Tuple_, Ts...> tuple_(
    Ts &&... xs) noexcept {
...
template <class T, class U,
          std::enable_if_t<
              disjunction_v<
                  ...
                  is_expr<std::decay_t<U>>, is_placeholder<std::decay_t<U>>>,
...
template <
    class T, class U,
    enabler_t<negation_v<
        concepts::required<std::decay_t<U>, concepts::iterable>>> = nullptr,
...
struct TupleDecomposerProxy {
  ...
  template <class T>
  constexpr TupleDecomposerProxy<Elem..., T> operator,(T &&t) {
    return TupleDecomposerProxy<Elem..., T>{
        {cranberries::back_magic::make_tuple_cat(std::move(elements),
                                                 std::forward<T>(t))}};
  }
};
...
struct ret_seq_for<Pack, std::index_sequence<I...>> {
  template <size_t N>
  using rip = std::conditional_t<
      std::is_same<pack_element_t<N, Pack>, placeholder_t<0>>::value,
      size_constant<0>, placeholder_t<N>>;
  using type = pack_t2v_transform_t<
      bind_1st<_index>::expr,
      pack_remove_t<size_constant<0>, type_pack<rip<I>...>>>;
};
...
struct TupleDecompose_<N, size_pack<Indices...>> {
  ...
  constexpr auto operator()(Tuple &&tup) const {
    static_assert(std::tuple_size<remove_cvr_t<Tuple>>::value == N,
                  "tuple size mismatch");
...
auto IntoDecompose(TupleDecomposerProxy<Elem...>) {
  return TupleDecompose_<
      sizeof...(Elem),
      typename ret_seq_for<TupleDecomposerProxy<remove_cvr_t<Elem>...>,
                           std::index_sequence_for<Elem...>>::type>{};
}

// template < class, size_t = 0, class = size_pack<>, class = size_pack<> >
// struct decompose_split;

// template < class Head, size_t I, class... Tail, size_t... Ret, size_t...
// Match > struct decompose_split<std::tuple<Head, Tail...>, I,
// size_pack<Ret...>, size_pack<Match...>> 	:
// std::conditional_t<is_ret<Head>::value,
// decompose_split<std::tuple<Tail...>, I+1, size_pack<Ret..., I>,
// size_pack<Match...>>,
// std::conditional_t<std::is_same<remove_cvr_t<Head>, placeholder_t<0>>::value,
// decompose_split<std::tuple<Tail...>, I + 1, size_pack<Ret...>,
// size_pack<Match...>>, decompose_split<std::tuple<Tail...>, I + 1,
// size_pack<Ret...>, size_pack<Match..., I>>
//	>>
//{};
...
template <class T, class U,
          enabler_t<is_placeholder_v<remove_cvr_t<T>> &&
                    is_placeholder_v<remove_cvr_t<U>>> = nullptr>
constexpr auto operator,(T &&t, U &&u) {
  return magic_arts::TupleDecomposerProxy<remove_cvr_t<T>, remove_cvr_t<U>>{
