#ifndef CRANBERRIES_FUNCTIONAL_BIND_EXPRESSION_HPP
#define CRANBERRIES_FUNCTIONAL_BIND_EXPRESSION_HPP
#include <functional>
#include <tuple>
...
struct placeholder_t {
  ...
  constexpr auto eval(Tuple &&tup) const {
    return std::get<N - 1>(std::forward<Tuple>(tup));
  }

  template <class... Args>
  constexpr auto operator()(Args &&... args) const {
    return std::get<N - 1>(std::forward_as_tuple(std::forward<Args>(args)...));
  }
};

template <>
struct placeholder_t<0> {};
...
struct make_placeholder_span_impl<value_pack<char, L...>, '\'', Rest...>
    : nested_type_class<placeholder_span<to_decimal<L...>::value,
                                         to_decimal<Rest...>::value>> {};

template <char... Chars>
using make_placeholder_span =
    make_placeholder_span_impl<value_pack<char>, Chars...>;
...
struct bind_expression_t {
  ...
  template <size_t N, class T,
            cranberries::enabler_t<is_placeholder_v<std::decay_t<
                std::tuple_element_t<N, std::tuple<L...>>>>> = nullptr>
  decltype(auto) get_(T &t) {
    return std::get<
        std::decay_t<std::tuple_element_t<N, std::tuple<L...>>>::value>(t);
  }

  template <size_t N, class T,
            cranberries::enabler_t<is_reference_wrapper_v<std::decay_t<
                std::tuple_element_t<N, std::tuple<L...>>>>> = nullptr>
  decltype(auto) get_(T &t) {
  ...
  template <size_t N, class T,
            cranberries::enabler_t<is_bind_expression_v<std::decay_t<
                std::tuple_element_t<N, std::tuple<L...>>>>> = nullptr>
  decltype(auto) get_(T &t) {
 ...
 public:
  ...
  bind_expression_t(F_ &&f, Tuple &&t)
      : f{std::forward<F_>(f)}, binds{std::forward<Tuple>(t)} {}
...
auto placeholder_span_expand(placeholder_span<First, Last>) {
  return placeholder_span_expand_impl(make_index_range<First, Last>{});
}

template <size_t I, class Tuple, class Head, class... Tail>
auto expand_(Tuple &&result, Head &&head, Tail &&... tail) {
  return expand_<I + 1>(
      std::tuple_cat(std::forward<Tuple>(result),
...
auto expand_(Tuple &&result, placeholder_t<0>, Tail &&... tail) {
  return expand_<I + 1>(std::tuple_cat(std::forward<Tuple>(result),
...
auto expand_(Tuple &&result, placeholder_t<N> p, Tail &&... tail) {
  return expand_<N + 1>(
      std::tuple_cat(std::forward<Tuple>(result), std::make_tuple(p)),
      std::forward<Tail>(tail)...);
}

template <size_t I, class Tuple, size_t First, size_t Last, class... Tail>
auto expand_(Tuple &&result, placeholder_span<First, Last> span,
             Tail &&... tail) {
  return expand_<Last + 1>(std::tuple_cat(std::forward<Tuple>(result),
...
auto expand_(Tuple &&result, placeholder_sequence<Id...>, Tail &&... tail) {
  return expand_<I>(std::tuple_cat(std::forward<Tuple>(result),
...
template <class F, class... Args>
bind_expression_t<F, Args...> make_bind_expresstion(F &&f,
                                                    std::tuple<Args...> &&t) {
...
constexpr auto bind(F &&f, Args &&... args) {
  return make_bind_expresstion(
      std::forward<F>(f),
      expand_<1>(std::tuple<>{}, std::forward<Args>(args)...));
}
...
template <char... Chars>
placeholder_t<to_decimal<Chars...>::value> constexpr operator"" _() {
  return {};
}

template <char... Chars>
auto operator"" _seq() {
  return typename make_placeholder_sequence<Chars...>::type{};
}

template <char... Chars>
auto operator"" _span() {
  return typename make_placeholder_span<Chars...>::type{};
}

}  // namespace placeholder_literals
}  // namespace literals
