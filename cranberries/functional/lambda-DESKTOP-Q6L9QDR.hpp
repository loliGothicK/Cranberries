...
#include "comprehension_variable.hpp"
#undef min
#undef max
namespace cranberries {
namespace expressions {
	class tuple_expr_tag {};
	class expr_base {};
	...
	struct ValWrapper_ : public expr_base
	{
		...
		auto eval(...) const noexcept { return value; }
	};


	template < size_t N >
	struct ValWrapper_<placeholder_t<N>> : public expr_base
	{
		ValWrapper_(placeholder_t<N>) {}
		template < class Tuple >
		auto eval(Tuple t) noexcept { return std::get<N-1>(t); }
	};

	template < class T >
	ValWrapper_<T> value_(T v) {
		return { v };
	}
	...
	private:
		...
		static constexpr auto calc(Accept ac, Tuple&& t, std::index_sequence<I...>) {
			return Func::apply((std::get<I>(t).eval(ac))...);
		}
	private:
		std::tuple<expr_bind_t<Args>...> args;

	public:
		ExprProxy_(expr_bind_t<Args>... a) : args(a...) {}

		ExprProxy_(const ExprProxy_&) = default;
		ExprProxy_& operator=(const ExprProxy_&) = default;

		template < class AcceptArgs >
		constexpr auto eval(AcceptArgs ac) {
			return calc(ac, args, std::index_sequence_for<Args...>{});
		}

		template < class... AcceptArgs >
		constexpr auto operator()(AcceptArgs... ac) {
			return calc(std::make_tuple(ac...), args, std::index_sequence_for<Args...>{});
		}
	};
	...
	template < class... Args >
	class ExprProxy_<Tuple_, Args...> : expr_base, tuple_expr_tag
	{
	private:
		template < class Accept, class Tuple, size_t... I >
		static constexpr auto calc(Accept ac, Tuple&& t, std::index_sequence<I...>) {
			return Tuple_::apply((std::get<I>(t).eval(ac))...);
		}
	private:
		std::tuple<expr_bind_t<Args>...> args;
	...
	public:
		...
		constexpr auto operator, (T&& x) {
			return ::cranberries::apply([&x](auto&&... va) {
				return ExprProxy_<Tuple_, Args..., T>{ std::forward<decltype(va)>(va)..., std::forward<T>(x) };
			}, args);
		}
	};
	...
	template < typename... Ts >
	inline constexpr expressions::ExprProxy_<expressions::Tuple_, Ts...>
	...
	template < class T, class U, std::enable_if_t<disjunction_v<
		expressions::is_expr<std::decay_t<T>>, is_placeholder<std::decay_t<T>>,
		expressions::is_expr<std::decay_t<U>>, is_placeholder<std::decay_t<U>>>, std::nullptr_t> = nullptr >
	...
	template < class T, class U, enabler_t<!std::is_base_of<expressions::tuple_expr_tag, T>::value> = nullptr,
		std::enable_if_t<disjunction_v<
			expressions::is_expr<std::decay_t<T>>, is_placeholder<std::decay_t<T>>,
			expressions::is_expr<std::decay_t<U>>, is_placeholder<std::decay_t<U>>>, std::nullptr_t> = nullptr >
		auto operator, (T&& t, U&& u) {
		return ::cranberries::tuple_(std::forward<T>(t), std::forward<U>(u));
	}
