...
namespace traversals {
	...
	namespace _detail {
		...
		struct _inf { using infinite_ = std::nullptr_t; };
		struct _fin { using finite_ = std::nullptr_t; };

		template < class, class = void >
		struct is_inf : std::false_type {};

		template < class T >
		struct is_inf<T, cranberries::void_t<typename remove_cvr_t<T>::infinite_>> : std::true_type {};

		template < class... Ts >
		struct disjunction_bound : std::conditional_t<disjunction_v< is_inf<Ts>... >, _inf, _fin> {};

		template < class... Ts >
		struct conjunction_bound : std::conditional_t<conjunction_v< negation<is_inf<Ts>>... >, _inf, _fin> {};


		struct _immut { using immutable_ = std::nullptr_t; };

		struct _mut{	using mutable_ = std::nullptr_t; };

		template < class, class = void >
		struct is_immut : std::false_type {};

		template < class T >
		struct is_immut<T, cranberries::void_t<typename remove_cvr_t<T>::immutable_>> : std::true_type {};

		template < class... Ts >
		struct mut_or : std::conditional_t<disjunction_v< is_immut<Ts>... >, _immut, _mut> {};

		template < class, class, class = void >
		struct size_impl_injection {};

		template < class Derived, class Trav >
		struct size_impl_injection<Derived, Trav,
			cranberries::void_t<decltype(std::declval<const Trav&>().size())>>
		{
			constexpr size_t size() const { return static_cast<const Derived*>(this)->traversal.size(); }
		};
	}
...
struct view_get {
	...
	static std::unique_ptr<polymorphic_view<typename remove_cvr_t<T>::value_type>>
		fwd(T&& trav) { return trav.fwd(); }
	template < class T >
	static std::unique_ptr<polymorphic_view<typename remove_cvr_t<T>::value_type>>
		rev(T&& trav) { return trav.rev(); }
};
...
public:
	template < class View >
	default_rev_view(std::unique_ptr<View> view)
		: data_{}
	{
		for (; !view->is_end(); view->next())
			data_.emplace_back(view->get());
		index_ = data_.size() - 1;
	}
protected:
