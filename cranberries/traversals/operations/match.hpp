...
class Match : private tag::adaptor_tag, private tag::action_tag {
  Pred pred;
...
template <class Pred>
class Match<Pred, match_mode::none> : private tag::adaptor_tag,
...
template <class Pred>
class Match<Pred, match_mode::any> : private tag::adaptor_tag,
                                     private tag::action_tag {
  Pred pred;
...
template <>
class Match<void, match_mode::regex> : private tag::adaptor_tag,
                                       private tag::action_tag {
  std::string regexp;
...
namespace collect {
template <class Pred>
Match<Pred, match_mode::all> all_match(Pred &&pred) {
  return {std::forward<Pred>(pred)};
}

template <class Pred>
Match<Pred, match_mode::none> none_match(Pred &&pred) {
  return {std::forward<Pred>(pred)};
}

template <class Pred>
Match<Pred, match_mode::any> any_match(Pred &&pred) {
  return {std::forward<Pred>(pred)};
}

inline Match<void, match_mode::regex> regex_match(std::string regexp) {
  return {regexp};
}
}  // namespace collect
