...
private:
	...
	public:
		...
		value_type get() const override final { return std::generate_canonical<RealType, std::numeric_limits<RealType>::digits>(engine); }
...
private:
	...
	public:
		template < class... Args >
		view_(Args&&... args) : dist{ std::random_device{}, std::forward<Args>(args)... } {}
		view_(const view_&) = default;
	...
	std::unique_ptr<polymorphic_view<value_type>> fwd() const {
		return ::cranberries::apply(
			[](auto&&... args) { return std::make_unique<view_>(std::forward<decltype(args)>(args)...); }
		, params);
	}
	std::unique_ptr<polymorphic_view<value_type>> rev() const {
		return ::cranberries::apply(
...
namespace create {
	...
	{
		return GenerateDistribution<Dist, decltype(std::forward_as_tuple(std::forward<Args>(args)...))>{
			std::forward_as_tuple(std::forward<Args>(args)...) };
	}

	template < class IntType,
		enabler_t<std::is_integral<IntType>::value> = nullptr >
	auto uniform_distribution(IntType a, IntType b)
	{
		return GenerateDistribution<std::uniform_int_distribution<IntType>, std::tuple<IntType, IntType>>{std::make_tuple(a,b)};
	}
	...
	{
		return GenerateDistribution<std::uniform_real_distribution<RealType>, std::tuple<RealType, RealType>>{std::make_tuple(a, b)};
	}
