...
template < class ValueType >
class Ranged<ValueType, true> final
...
private:
	...
	public:
		fwd_view(ValueType init, ValueType upper) : value_{ init }, upper_bound{ upper } {}
	...
	public:
		rev_view(value_type init, value_type upper) : value_{ --upper }, lower_bound{ --init } {}
	...
	::std::unique_ptr< ::cranberries::traversals::polymorphic_view<value_type >> rev() const {
...
template < class ValueType >
class Ranged<ValueType, false> final
	: private tag::sentinel_traversal_tag
	, public reversable_traversal
{
	friend struct view_get;
...
private:
	class fwd_view final : public polymorphic_view<value_type> {
		...
		value_type lower_bound;
	public:
		fwd_view(value_type init, value_type upper) : value_{ init }, lower_bound{ upper } {}
		fwd_view(const fwd_view&) = default;
	protected:
		value_type get() const override final { return value_; }
		void next() override final { ++value_; }
...
namespace create {
	...
	>
	Ranged<ValueType>
	semi_irange(ValueType init, ValueType upper_bound)
	...
	>
	Ranged<ValueType>
	irange(ValueType init, ValueType upper_bound)
