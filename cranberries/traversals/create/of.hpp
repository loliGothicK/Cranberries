...
class RangeWrapper : private tag::sentinel_traversal_tag,
 ...
 private:
  class forward_view final : public polymorphic_view<value_type> {
    decltype(
        ::cranberries::back_magic::cbegin(std::declval<const Range &>())) iter;
    decltype(
        ::cranberries::back_magic::cend(std::declval<const Range &>())) last;

   public:
    template <class Iterator>
    forward_view(Iterator first, Iterator last) : iter{first}, last{last} {}
  ...
  class reverse_view final : public polymorphic_view<value_type> {
    decltype(
        ::cranberries::back_magic::crbegin(std::declval<const Range &>())) iter;
    decltype(
        ::cranberries::back_magic::crend(std::declval<const Range &>())) last;

   public:
    template <class Iterator>
    reverse_view(Iterator first, Iterator last) : iter{first}, last{last} {}
...
class RangeWrapper<Range,
                   void_t<decltype(back_magic::crbegin(std::declval<Range>()))>>
...
namespace create {
template <class Range>
auto of(Range &&traversal) {
  return RangeWrapper<Range>{std::forward<Range>(traversal)};
}

template <class T>
auto of(std::initializer_list<T> il) {
  return RangeWrapper<std::initializer_list<T>>{il};
}
}  // namespace create
