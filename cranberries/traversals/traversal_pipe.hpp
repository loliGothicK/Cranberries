...
namespace _detail {
	...
	struct trav_require
		: std::is_base_of<forward_traversal, remove_cvr_t<Trav>>
	{};

	template < class Trav, class X >
	struct trav_require<Trav,X,
		cranberries::void_t<
			typename remove_cvr_t<X>::template requires<remove_cvr_t<Trav>>
		>>
		: std::true_type
	{};
...
decltype(auto) operator<= (Range&& traversal, Adaptor&& adaptor)
	noexcept(noexcept(std::forward<Adaptor>(adaptor)(std::forward<Range>(traversal))))
{
