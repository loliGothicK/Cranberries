...
namespace view {
	...
	auto concat(RightRange&& traversal) { return make_proxy<Concat>::embedded( std::forward<RightRange>(traversal) ); }

	auto concat_ = [](auto&& traversal) { concat(std::forward<decltype(traversal)>(traversal)); };
}
