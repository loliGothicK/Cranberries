...
public:
	...
	Filter(Traversal traversal, Pred pred)
		: traversal{ traversal }
		, pred{ pred }
	{}
...
private:
	...
	public:
		...
		void next() override final { view->next();while( !pred(view->get()) ) view->next(); }
...
class Filter<Traversal, Pred, false>
	: private tag::sentinel_traversal_tag
...
namespace _proxy {
	...
	public:
		...
		template < class Trav >
		auto operator()(Trav&& trav) { return Filter<Trav, Pred, std::is_base_of<reversable_traversal, Trav>::value>{
...
namespace view {
	template < class Pred >
	inline auto filter(Pred pred) noexcept { return _proxy::FilterX<Pred>{ std::forward<Pred>(pred) }; }
}
