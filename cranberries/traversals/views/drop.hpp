...
namespace _detail {
	...
	struct size_impl_injection_for_drop<Drop<Traversal,true>,
		::cranberries::void_t<decltype( 1 != std::declval<const Traversal&>().size())>
	>
	{
		constexpr size_t size() const {
			auto has  = static_cast<const Drop<Traversal, true>*>(this)->traversal.size();
...
public:
	...
	Drop(Traversal traversal, size_t n)
		: traversal{ traversal }
		, n(n) {}
...
namespace _proxy {
	...
	public:
		...
		template < class Trav >
		auto operator()(Trav&& trav) { return Drop<Trav, std::is_base_of<reversable_traversal, Trav>::value>{
