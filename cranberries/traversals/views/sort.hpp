...
class Sorted : private tag::sentinel_traversal_tag,
               private reversable_traversal,
               public _detail::size_impl_injection<Sorted<Traversable, Pred>,
                                                   Traversable> {
 ...
 public:
  ...
  Sorted(Trav_ &&traversal, Pred_ &&pred)
      : traversal{std::forward<Trav_>(traversal)},
...
template <class Pred>
auto sort(Pred &&pred) {
  return _proxy::SortX<Pred>{std::forward<Pred>(pred)};
}
}  // namespace view
