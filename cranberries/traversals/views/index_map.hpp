...
template <class Traversal, class F, typename IndexType, class Next, bool = true>
class IndexMap : private tag::sentinel_traversal_tag,
                 public reversable_traversal {
  friend struct view_get;

 public:
  using value_type = cranberries::invoke_result_t<
      F, IndexType, typename remove_cvr_t<Traversal>::value_type>;
 ...
 private:
  class _view final : public polymorphic_view<value_type> {
    std::unique_ptr<
        polymorphic_view<typename remove_cvr_t<Traversal>::value_type>>
        view_;
    IndexType index;
    const F &f;
    Next _next;
   ...
   public:
    _view(std::unique_ptr<
              polymorphic_view<typename remove_cvr_t<Traversal>::value_type>>
              p,
          const IndexType &init, const F &f, const Next &next)
  ...
  struct rev_impl {
    ...
    static constexpr auto invoke(Trav &&trav, Args &&... args) {
      return std::make_unique<_view>(view_get::fwd(std::forward<Trav>(trav)),
                                     std::forward<Args>(args)...);
    }
  };
  template <class T>
  struct rev_impl<
      T, cranberries::void_t<decltype(std::declval<T const &>().size())>> {
    template <class Trav, class F_, class Next_>
    static constexpr auto invoke(Trav &&trav, IndexType index, F &&f,
                                 Next &&incr) {
      for (size_t i{}; i < trav.size(); ++i) index = incr(index);
      return std::make_unique<default_rev_view<value_type>>(
          view_get::rev(std::forward<Trav>(trav)), index, std::forward<F_>(f),
  ...
  std::unique_ptr<polymorphic_view<value_type>> rev() const {
    return rev_impl<remove_cvr_t<Traversal>>::invoke(
        std::forward<Traversal>(traversal), ini, std::forward<F>(f),
        std::forward<Next>(incr));
  }
...
template <class Traversal, class F, typename IndexType, class Next>
class IndexMap<Traversal, F, IndexType, Next, false>
    : private tag::sentinel_traversal_tag, public forward_traversal {
  friend struct view_get;
 ...
 private:
  class _view final : public polymorphic_view<value_type> {
   ...
   public:
    _view(std::unique_ptr<
              polymorphic_view<typename remove_cvr_t<Traversal>::value_type>>
              p,
          const F &f, const IndexType &init, const Next &next)
  ...
  std::unique_ptr<polymorphic_view<value_type>> fwd() const {
    return std::make_unique<_view>(view_get::fwd(traversal), std::forward<F>(f),
                                   ini, std::forward<Next>(incr));
  }
...
class IndexMapX : tag::adaptor_tag {
 ...
 public:
  ...
  auto operator()(Traversable &&trav) {
    return IndexMap<Traversable, F, IndexType, Next,
                    std::is_base_of<reversable_traversal,
                                    remove_cvr_t<Traversable>>::value>{
        std::forward<Traversable>(trav), std::forward<F>(f), ini, next};
  }
};
...
class IndexMapX<F, IndexType, void> : tag::adaptor_tag {
 ...
 public:
  ...
  template <class Traversable>
  auto operator()(Traversable &&trav) {
    return IndexMap<Traversable, F, IndexType, cranberries::pre_increment<>,
                    std::is_base_of<reversable_traversal,
                                    remove_cvr_t<Traversable>>::value>{
        std::forward<Traversable>(trav), std::forward<F>(f), ini};
  }
};
...
class IndexMapX<F, void, void> : tag::adaptor_tag {
 ...
 public:
  ...
  template <class Traversable>
  auto operator()(Traversable &&trav) {
    return IndexMap<Traversable, F, size_t, cranberries::pre_increment<>,
                    std::is_base_of<reversable_traversal,
                                    remove_cvr_t<Traversable>>::value>{
...
template <typename F>
auto index_map(F &&f) {
  return _proxy::IndexMapX<F &&>{std::forward<F>(f)};
}

template <typename F, class IndexType>
auto index_map(F &&f, const IndexType &ini) {
  return _proxy::IndexMapX<F &&, const IndexType &>{std::forward<F>(f), ini};
}

template <typename F, class IndexType, class Next>
auto index_map(F &&f, const IndexType &ini, Next &&next) {
  return _proxy::IndexMapX<F &&, const IndexType, Next &&>{
      std::forward<F>(f), ini, std::forward<Next>(next)};
}
}  // namespace view
