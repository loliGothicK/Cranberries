...
struct size_impl_injection_for_zipwith<
    ZipWith<Left, Right>,
    ::cranberries::void_t<decltype(std::declval<const Left &>().size() ==
                                   std::declval<const Right &>().size())>> {
...
class ZipWith
  ...
  template <class, class>
  friend struct _detail::size_impl_injection_for_zipwith;

 public:
  using value_type = std::tuple<typename remove_cvr_t<Left>::value_type,
  ...
  ZipWith(Left left, Right right) : left{left}, right{right} {}
 ...
 private:
  class view_ final : public polymorphic_view<value_type> {
    std::unique_ptr<polymorphic_view<typename remove_cvr_t<Left>::value_type>>
        left_;
    std::unique_ptr<polymorphic_view<typename remove_cvr_t<Right>::value_type>>
        right_;

   public:
    view_(std::unique_ptr<
              polymorphic_view<typename remove_cvr_t<Left>::value_type>>
              left,
          std::unique_ptr<
              polymorphic_view<typename remove_cvr_t<Right>::value_type>>
              right)
        : left_{std::move(left)}, right_{std::move(right)} {}
    value_type get() const override final {
      return std::make_tuple(left_->get(), right_->get());
    }
...
class ZipWithX : tag::adaptor_tag {
 ...
 public:
  ...
  auto operator()(Left &&left) {
    return ZipWith<Left, Right>{std::forward<Left>(left),
...
namespace view {
template <class Right>
auto zip_with(Right &&traversal) {
  return _proxy::ZipWithX<Right>{std::forward<Right>(traversal)};
}
}  // namespace view
...
class Zipped<std::index_sequence<I...>, Traversals...>
    : private tag::sentinel_traversal_tag, private forward_traversal {
  friend struct view_get;

 public:
  using value_type =
      std::tuple<typename std::decay_t<Traversals>::value_type...>;
  using iterator = sentinel_iterator<value_type>;

  template <class... Traversals_>
  Zipped(Traversals_ &&... traversals)
      : traversals_{
            std::forward_as_tuple(std::forward<Traversals_>(traversals)...)} {}
  ...
  class view_ final : public polymorphic_view<value_type> {
   ...
   public:
    ...
    virtual value_type get() const override final {
      return std::forward_as_tuple(*std::get<I>(iters_)...);
    }
...
auto make_zipped(Traversals &&... traversals) {
  return Zipped<std::index_sequence_for<Traversals...>, Traversals...>{
      std::forward<Traversals>(traversals)...};
}
}  // namespace view
