...
class DropWhile<Traversal, Pred, false>
	: private tag::sentinel_traversal_tag
...
namespace _proxy {
	...
	public:
		...
		template < class Trav >
		auto operator()(Trav&& trav) { return DropWhile<Trav, Pred, std::is_base_of<reversable_traversal, Trav>::value>{
...
namespace view {
	template < class Pred >
	inline auto drop_while(Pred pred) noexcept { return _proxy::DropWhileX<Pred>{ std::forward<Pred>(pred) }; }
}
