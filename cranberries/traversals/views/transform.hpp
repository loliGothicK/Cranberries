...
protected:
	...
	virtual std::unique_ptr<polymorphic_view<value_type>> rev() const override final {
...
namespace _proxy {
	...
	public:
		...
		template < class Traversable, typename T = typename remove_cvr_t<Traversable>::value_type >
		auto operator()(Traversable&& trav) {
			return Map< T, F, std::is_base_of<reversable_traversal<T>, remove_cvr_t<Traversable>>::value >{
				std::make_shared<remove_cvr_t<Traversable>>( std::forward<Traversable>(trav) ), std::forward<F>(f)
			};
		}
	};
}
