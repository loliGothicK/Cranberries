...
class Indexed
 ...
 public:
  using value_type =
      typename std::tuple<size_t, typename remove_cvr_t<Traversal>::value_type>;
  using iterator = sentinel_iterator<value_type>;
  using reverse_iterator = sentinel_iterator<value_type>;

  Indexed(Traversal traversal, size_t n) : traversal{traversal}, index(n) {}
  ...
  class view_ final : public polymorphic_view<value_type> {
   ...
   public:
    view_(std::unique_ptr<
              ...
              _view,
          size_t n)
        : view{std::move(_view)}, index{n} {}
    value_type get() const override final {
      return std::make_tuple(index, view->get());
    }
...
class Indexed<Traversal, false> : private tag::sentinel_traversal_tag,
...
struct IndexedX : tag::adaptor_tag {
 ...
 public:
  ...
  auto operator()(Trav &&trav) {
    return Indexed<Trav, std::is_base_of<reversable_traversal, Trav>::value>{
