...
struct size_impl_injection_for_chunk<
  ...
  size_t size() const {
    return (static_cast<const Chunk<N, Trav> *>(this)->trav).size() / N;
  }
};

template <class Trav>
struct size_impl_injection_for_chunk<
    Chunk<static_cast<size_t>(-1), Trav>,
    ::cranberries::void_t<decltype(std::declval<const Trav &>().size())>> {
  size_t size() const {
    return (static_cast<int>(
                (static_cast<const Chunk<static_cast<size_t>(-1), Trav> *>(this)
                     ->trav)
                    .size()) -
            1) /
               (static_cast<const Chunk<static_cast<size_t>(-1), Trav> *>(this)
...
class Chunk
  ...
  friend struct _detail::size_impl_injection_for_chunk;

 public:
  using value_type =
      std::array<typename ::cranberries::remove_cvr_t<Traversal>::value_type,
                 N>;
  ...
  class fwd_view final : public polymorphic_view<value_type> {
   ...
   public:
    fwd_view(std::unique_ptr<
             polymorphic_view<typename remove_cvr_t<Traversal>::value_type>>
                 p)
        : view{std::move(p)}, chunk{{}}, eov{view->is_end()} {
      for (auto &e : chunk) {
        if (view->is_end()) {
          eov = true;
          break;
...
class Chunk<static_cast<size_t>(-1), Traversal>
    : private tag::sentinel_traversal_tag,
      public reversable_traversal,
      public _detail::conjunction_bound<Traversal>,
      public _detail::size_impl_injection_for_chunk<
          Chunk<static_cast<size_t>(-1), Traversal>> {
  friend struct view_get;
  template <class, class>
  friend struct _detail::size_impl_injection_for_chunk;

 public:
  using value_type = ::cranberries::readonly_span_t<
      const std::vector<typename remove_cvr_t<Traversal>::value_type>>;
  using iterator = sentinel_iterator<value_type>;
  using reverse_iterator = sentinel_iterator<value_type>;
  ...
  class fwd_view final : public polymorphic_view<value_type> {
   ...
   public:
    fwd_view(std::unique_ptr<
                 ...
                 p,
             size_t n)
        : view{std::move(p)}, n{n}, chunk(n), eov{view->is_end()} {
   ...
   protected:
    value_type get() const override final {
      return ::cranberries::readonly_span(chunk, 0, n);
    }
    void next() override final {
      eov = eov_next;
      for (size_t i{}; i < n; ++i, view->next()) {
...
namespace _proxy {
template <size_t N>
class ChunkX : private tag::adaptor_tag {
 public:
  ChunkX() = default;
...
template <>
class ChunkX<static_cast<size_t>(-1)> : private tag::adaptor_tag {
  size_t n;
 ...
 public:
  ...
  template <class Traversal>
  auto operator()(Traversal &&traversal) {
    return Chunk<static_cast<size_t>(-1), Traversal>{
        std::forward<Traversal>(traversal), n};
  }
};
}  // namespace _proxy
namespace view {
template <size_t N, enabler_t<bool(N)> = nullptr>
_proxy::ChunkX<N> chunk() {
  return {};
}
