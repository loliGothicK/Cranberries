...
class ComprehensionGenerator
 ...
 public:
  using value_type = decltype(::cranberries::back_magic::make_tuple_cat(
      std::declval<typename std::decay_t<Traversable1>::value_type>(),
      std::declval<typename std::decay_t<Traversable2>::value_type>()));
  ...
  ComprehensionGenerator(T1 &&tr1, T2 &&tr2)
      : traversal1{std::forward<T1>(tr1)}, traversal2{std::forward<T2>(tr2)} {}
 ...
 private:
  class view_ final
   ...
   public:
    view_(std::unique_ptr<traversals::polymorphic_view<value_type_1>> tr1,
          std::vector<value_type_2> tr2)
        : view{std::move(tr1)},
...
class ComprehensionGenerator<
    Traversal,
    std::enable_if_t<!is_tuple_v<typename remove_cvr_t<Traversal>::value_type>>>
    : private ::cranberries::traversals::tag::sentinel_traversal_tag,
      private ::cranberries::traversals::forward_traversal {
  friend struct view_get;
 ...
 public:
  ...
  template <class Trav>
  ComprehensionGenerator(Trav &&trav) : traversal{std::forward<Trav>(trav)} {}
...
auto make_comprehension_generator(Tr1 &&tr1, Tr2 &&tr2) {
  return ComprehensionGenerator<Tr1, Tr2>{std::forward<Tr1>(tr1),
                                          std::forward<Tr2>(tr2)};
}
