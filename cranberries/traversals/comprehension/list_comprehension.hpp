...
decltype(auto) operator&(Traversal1 &&lhs, Traversal2 &&rhs) {
  return _detail::make_comprehension_generator(std::forward<Traversal2>(rhs),
                                               std::forward<Traversal1>(lhs));
}

// template < class Traversal, class Pred,
//	concepts::concept_required<Traversal, concepts::iterable> = nullptr,
//	enabler_t<is_callable_v<Pred(typename
// std::decay_t<Traversal>::value_type)>, bool> = nullptr> decltype(auto)
// operator== (Traversal&& trv, Pred&& pred)
//{
//	return
//::cranberries::traversals::view::filter(std::forward<Pred>(pred))(std::forward<Traversal>(trv));
//}
...
class list_comprehension_t
 ...
 public:
  using value_type = decltype(::cranberries::apply(
      std::declval<ET>(),
      std::declval<typename std::decay_t<Traversable>::value_type>()));
  using iterator = ::cranberries::traversals::sentinel_iterator<value_type>;

  template <class Trav>
  list_comprehension_t(ET expr, Trav &&gen)
      : expr{expr}, gen{std::forward<Trav>(gen)} {}

 private:
  class view_ final
      : public ::cranberries::traversals::polymorphic_view<value_type> {
    ET expr_;
    std::unique_ptr<::cranberries::traversals::polymorphic_view<
        typename ::cranberries::remove_cvr_t<Traversable>::value_type>>
        gen_;
...
template <class ET, class Traversable>
list_comprehension_t<ET, Traversable> make_list_comprehension(
    ET &&expr, Traversable &&gen) {
...
template <class ET, class Traversable,
          enabler_t<conjunction_v<
              ...
              cranberries::is_expression<std::decay_t<ET>>>> = nullptr>
auto operator|(ET &&expr, Traversable &&gen) {
  return make_list_comprehension(std::forward<ET>(expr),
                                 std::forward<Traversable>(gen));
}

template <
    class ET, class Traversable,
    enabler_t<conjunction_v<
        concepts::required<Traversable, concepts::iterable>,
