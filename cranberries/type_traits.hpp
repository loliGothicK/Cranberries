...
namespace _detail {
	...
	namespace _detail {
		...
		struct invoke_result<decltype(void(_detail::INVOKE(std::declval<F>(), std::declval<Args>()...))), F, Args...>
		{
			using type = decltype(_detail::INVOKE(std::declval<F>(), std::declval<Args>()...));
		};
	} // namespace _detail
