...
template <class T>
class identity : nested_type_class<T> {};
...
struct max_sizeof<Head, Tail...>
    : size_constant<(sizeof(Head) > max_sizeof<Tail...>::value)
                        ? sizeof(Head)
...
struct min_sizeof<Head, Tail...>
    : size_constant<(sizeof(Head) > min_sizeof<Tail...>::value)
                        ? sizeof(Head)
                        : min_sizeof<Tail...>::value> {};
...
struct enable_std_begin_end<
    T, cranberries::void_t<decltype(std::begin(std::declval<T &>()),
...
struct enable_adl_begin_end<
    T, cranberries::void_t<decltype(begin(std::declval<T &>()),
                                    end(std::declval<T &>()))>>
    : std::true_type {};
...
template <typename T>
using is_range = disjunction<cranberries_magic::enable_std_begin_end<T>,
...
struct is_equality_comparable_to<
    T, U, void_t<decltype(std::declval<T &>() == std::declval<U &>())>>
    : std::true_type {};

template <typename T, typename U>
constexpr bool is_equality_comparable_to_v =
    is_equality_comparable_to<T, U>::value;
...
struct is_referenceable
    : public std::integral_constant<
          bool,
          !(std::is_same<decltype(_detail::is_referenceable_helper::test<T>(0)),
...
template <typename T>
struct is_unknown_bound_array : public std::false_type {};
template <typename T>
struct is_unknown_bound_array<T[]> : public std::true_type {};

template <typename T>
struct is_swappable_applicable
    : public disjunction<is_complete_type<T>, std::is_void<T>,
...
struct is_swappable_checker {
  constexpr static bool value =
      std::is_same<decltype(is_swappable_helper::test<T, U>(0)),
                   std::true_type>::value;
};
...
struct is_swappable_with
    : public std::integral_constant<
          bool, _detail::is_swappable_checker<T, U>::value> {
...
template <typename T>
struct is_swappable_referenceable_checker<T, false> : public std::false_type {};

}  // namespace _detail

template <typename T>
struct is_swappable : public _detail::is_swappable_referenceable_checker<
                          T, is_referenceable<T>::value> {};
...
struct is_nothrow_swappable_checker {
  constexpr static bool value =
      std::is_same<decltype(is_nothrow_swappable_helper::test<T, U>(0)),
                   std::true_type>::value;
};
...
struct is_nothrow_swappable_with
    : public std::integral_constant<
          bool, _detail::is_nothrow_swappable_checker<T, U>::value> {
  static_assert(_detail::is_swappable_applicable<T>::value,
                "the argument of T would cause undefined behavior");
  static_assert(_detail::is_swappable_applicable<U>::value,
                "the argument of U would cause undefined behavior");
...
struct is_nothrow_swappable
    : public _detail::is_nothrow_swappable_referenceable_checker<
          T, is_referenceable<T>::value> {};
...
template <class T>
struct is_dereferencable<T, void_t<decltype(*std::declval<const T &>())>>
    : std::true_type {};
...
struct has_value_type<
    T, void_t<decltype(std::declval<typename std::decay_t<T>::value_type>())>>
    : std::true_type {};
...
template <class T>
struct has_value_field<T, void_t<decltype(std::decay_t<T>::value)>>
    : std::true_type {};
...
struct has_type_member<
    T, void_t<decltype(std::declval<typename std::decay_t<T>::type>())>>
    : std::true_type {};
...
template <class... Ts>
struct element_type_of<std::tuple<Ts...>> : std::common_type<Ts...> {};
...
template <typename T, bool B = is_range_v<T>>
...
template <class, class R = void, class = void>
struct is_callable : std::false_type {};

template <class Fn, class... ArgTypes, class R>
struct is_callable<Fn(ArgTypes...), R,
                   cranberries::void_t<decltype(
                       std::declval<Fn>()(std::declval<ArgTypes>()...))>>
    : std::is_convertible<
          decltype(std::declval<Fn>()(std::declval<ArgTypes>()...)), R> {};

template <class Fn, class... ArgTypes>
struct is_callable<Fn(ArgTypes...), return_any,
                   cranberries::void_t<decltype(std::declval<Fn>()(
                       std::declval<ArgTypes>()...))>> : std::true_type {};
...
struct is_nothrow_callable<Fn(ArgTypes...), R,
    ...
    : cranberries::bool_constant<noexcept(
          std::declval<Fn>()(std::declval<ArgTypes>()...))> {};

template <class Fn, class... ArgTypes>
struct is_nothrow_callable<Fn(ArgTypes...), return_any,
                           cranberries::void_t<decltype(std::declval<Fn>()(
                               std::declval<ArgTypes>()...))>>
    : cranberries::bool_constant<noexcept(
          std::declval<Fn>()(std::declval<ArgTypes>()...))> {};
...
struct generate_tuple {
  ...
  using type =
      std::conditional_t<N % 2 == 0,
                         decltype(std::tuple_cat(std::declval<partial_type>(),
                                                 std::declval<partial_type>())),
                         decltype(
                             std::tuple_cat(std::declval<partial_type>(),
                                            std::declval<partial_type>(),
...
constexpr auto INVOKE(T Base::*pmf, Derived &&ref, Args &&... args) noexcept(
...
struct invoke_result<decltype(void(_detail::INVOKE(std::declval<F>(),
                                                   std::declval<Args>()...))),
                     F, Args...> {
  using type =
      decltype(_detail::INVOKE(std::declval<F>(), std::declval<Args>()...));
};
}  // namespace _detail

template <class F, class... ArgTypes>
struct invoke_result : _detail::invoke_result<void, F, ArgTypes...> {};
