...
class optional {
 ...
 public:
  ...
  constexpr operator bool() const { return init_; }
...
class optional<T &> {
 ...
 public:
  ...
  constexpr T &operator*() const { return storage_; }
  constexpr operator bool() const { return init_; }
  constexpr T value_or(const T &or_) const { return init_ ? storage_ : or_; }
};

template <class T>
optional<T> some(T &&some) {
