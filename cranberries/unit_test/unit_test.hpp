...
#ifndef _WIN32
  static void Red() { std::cout << "\x1b[31m"; }
  static void Green() { std::cout << "\x1b[32m"; }
  static void Yellow() { std::cout << "\x1b[33m"; }
  static void Blue() { std::cout << "\x1b[34m"; }
  static void Reset() { std::cout << "\x1b[0m"; }
#else
  static void Reset() {
    HANDLE handle = GetStdHandle(STD_OUTPUT_HANDLE);
    SetConsoleTextAttribute(
        handle, FOREGROUND_RED | FOREGROUND_BLUE | FOREGROUND_GREEN);
  }
...
struct assert_except : public std::runtime_error {
  assert_except(std::string info) : std::runtime_error{info} {}
};
...
test_status &operator&=(test_status &a, test_status b) { return a = a & b; }
...
class test_result_t {
 ...
 public:
  ...
  test_result_t(std::string msg) : status_{test_status::failed}, what_{msg} {}
  test_result_t(const char *msg) : status_{test_status::failed}, what_{msg} {}
...
class TestMethod {
 ...
 public:
  ...
  TestMethod(std::shared_ptr<F> f, std::string _)
      : f_{std::move(f)}, label_{_} {}
...
template <class F,
          enabler_t<!std::is_base_of<detail_::test_method_tag,
                                     std::decay_t<F>>::value> = nullptr>
TestMethod<F> make_test_method(F &&f) {
  return {std::make_shared<F>(std::forward<F>(f)), "test method"};
}

template <class F, enabler_t<std::is_base_of<detail_::test_method_tag,
                                             std::decay_t<F>>::value> = nullptr>
TestMethod<F> make_test_method(F &&f) {
...
class UnitTestContainer {
  ...
  decltype(std::chrono::high_resolution_clock::now())
      start;  // time point of first push
 ...
 public:
  ...
  auto collect() {
    ...
    ConsoleTestColor::Reset();
    logger << " |\n"
           << name << " End : Total Real Time = " << std::fixed
           << std::setprecision(5);
    (total_.count() < 1000 ? (logger << total_.count() << " [ms]")
  ...
  void operator|(result_collect) {
    if (this->collect())
      throw detail_::assert_except{this->name + "container failed!"};
  }
  ...
  decltype(auto) operator%(F &&f) & {
    ...
    methods.emplace_back(
        std::async(std::launch::async,  // !explicit specified async
                   [&, f = detail_::make_test_method(std::move(f))
                               .index(++index)]() mutable -> test_status {
                     f.exe();
  ...
  decltype(auto) operator%(detail_::labeled_method<F> &&lm) & {
    ...
    methods.emplace_back(
        std::async(std::launch::async,  // !explicit specified async
                   [&, f = make_test_method(std::move(lm.f_), lm.label_)
                               .index(++index)]() mutable -> test_status {
                     f.exe();
                     auto_lock(mtx_), f.print(logger);
                     return f.status();
...
UnitTestContainer<LOGGER_> make_unit_test_container(
    LOGGER_ lg = default_logger, std::string name = "UNIT TEST") {
  return {lg, name};
}

template <class LOGGER_ = logger<>>
UnitTestContainer<LOGGER_> make_unit_test_container(
    std::string name = "UNIT TEST", LOGGER_ lg = default_logger) {
  return {lg, name};
}
...
struct enable_labeled {
  auto labeled(std::string label) {
    return labeled_method<Derived>{std::move(*static_cast<Derived *>(this)),
                                   label};
  }
};

template <class Expr, class Generator>
class ScrambleExe : public enable_labeled<ScrambleExe<Expr, Generator>>,
 ...
 public:
  ScrambleExe(Expr expr, Generator gen, size_t n)
      : expr_{expr}, generator_{gen}, times_{n} {}

  test_result_t operator()() {
    for (size_t i{}; i < times_; ++i) cranberries::apply(expr_, generator_());
    return test_status::passed;
  }
...
class PermutationExe
 ...
 public:
  PermutationExe(Expr expr, Require require, Tuple t)
      : expr_{expr}, require_{require}, sample_(t) {}
  ...
  test_result_t execute(std::index_sequence<I...>) {
    ...
    } while (cranberries::next_partial_permutation(
        indices.begin(), indices.begin() + N, indices.end()));
    if (!exit_status)
  ...
  test_result_t operator()() { return execute(std::make_index_sequence<N>{}); }

  static std::string label() { return "permutation execute"; }
};

template <size_t N, class Expr, class Sample>
class CombinationExe : private detail_::test_method_tag,
                       public enable_labeled<CombinationExe<N, Expr, Sample>> {
  Expr expr_;
  Sample sample_;

 public:
  CombinationExe(Expr expr, Sample t) : expr_{expr}, sample_(t) {}
  ...
  test_result_t execute(std::index_sequence<I...>) {
    ...
    } while (cranberries::next_combination(indices.begin(), indices.begin() + N,
                                           indices.end()));
    return test_status::passed;
  }
...
class AreEqual : private detail_::test_method_tag,
...
class RangeEqual : private detail_::test_method_tag,
 ...
 public:
  ...
  RangeEqual(
      Range_ &&range,
      std::initializer_list<typename std::decay_t<Range>::value_type> expect)
      : range_{std::forward<Range_>(range)}, expect_(expect) {}
  ...
  std::string info() {
    return std::string{"Info> Assertion failure: "} + "'" +
           info_::to_string(expect_) + "'" + " expected but " + "'" +
           info_::to_string(range_) + "'" + " actual.";
  }
...
class RangeMatch : private detail_::test_method_tag,
 ...
 public:
  ...
  RangeMatch(Range_ &&range, Pred pred)
      : range_{std::forward<Range_>(range)}, pred_{pred} {}
...
static detail_::AreEqual<Value, Value> are_equal(
    Value &&actual, std::add_const_t<Value> expect) {
  return {std::forward<Value>(actual), expect};
}

template <class Range>
static auto range_equal(
    Range &&range,
    std::initializer_list<typename std::decay_t<Range>::value_type> expected)
    -> detail_::RangeEqual<Range> {
  return {std::forward<Range>(range), expected};
}

template <class Expr, class Pred>
static auto range_match(Expr &&expr, Pred &&pred)
    -> detail_::RangeMatch<Expr, Pred> {
  return {std::forward<Expr>(expr), std::forward<Pred>(pred)};
}

template <class Except, class Expr>
static detail_::ExactThrow<Expr, Except> excact_throw(Expr &&expr) {
  return {std::forward<Expr>(expr)};
}

template <size_t N, class F, class Require, class... Samples>
static auto permutation_execute(F &&target, Require &&req,
                                Samples &&... samples)
    -> detail_::PermutationExe<
        N, F, Require,
        decltype(make_array(std::forward<Samples>(samples)...))> {
  return {std::forward<F>(target), std::forward<Require>(req),
          make_array(std::forward<Samples>(samples)...)};
}
