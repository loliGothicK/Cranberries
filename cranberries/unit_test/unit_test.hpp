...
#ifndef _WIN32
  static void Red() { std::wcout << "\x1b[31m"; }
  static void Green() { std::wcout << "\x1b[32m"; }
...
template <class Range,
          class Elem = typename std::decay_t<Range>::value_type,
    ...
    std::declval<std::stringstream&>() << std::declval<typename std::decay_t<Range>::value_type>())>>
{
  ...
  static std::string invoke(T1&& range_, T2&& expect_)
  {
    using printable_extensons::operator<<;
    return
      std::string{"Info> Assertion failure: "}
 ...
 public:
  ...
  test_result_t operator()()
  {
    ...
    for (auto&& e : range_) {
      exit_status &= pred_(e);
      if (!exit_status)
        return infomation::RangeMatchInfo<Range>::invoke(nth, e);
      nth++;
    }
    return exit_status;
  }
...
template <bool B>
class AssertBool : private detail_::test_method_tag,
                   public enable_labeled<AssertBool<B>> {
  bool expect_;

 public:
  AssertBool(bool a) : expect_{a} {}
  ...
  test_result_t operator()()
  {
    if (B == expect_)
      ...
      return std::string{"Info> Assertion failure: expect "} +
             (B ? std::string{"true"} : std::string{"false"}) + "but " +
             (!B ? std::string{"true"} : std::string{"false"}) + " returned.";
  }
  ...
  , public enable_labeled<AreEqualFloatingPoints<FloatType>>
{
 ...
 public:
  AreEqualFloatingPoints(FloatType actual, FloatType expected, FloatType eps)
      : actual_{actual}, expected_{expected}, eps_{eps} {}

  test_result_t operator()() {
    if (expected_ - eps_ <= actual_ && actual_ <= expected_ + eps_)
      return test_status::passed;
    else
      return [&] {
        std::stringstream ss;
        ss << "Info> Assertion failure: expect " << expected_ << " eps[" << eps_
           << "] but " << actual_ << " returned.";
        return ss.str();
      }();
  }

  static std::string label() { return "are equal floating points"; }
};
	...
	, public enable_labeled<AssertRelational<A, E, Pred>>
{
 ...
 public:
  AssertRelational(A const& a, E const& e)
      : actual_{a}, expected_{e}, pred_{} {}

  template <class _Pred>
  AssertRelational(A const& a, E const& e, _Pred&& pred)
      : actual_{a}, expected_{e}, pred_{std::forward<_Pred>(pred)} {}

  test_result_t operator()()
  {
    if (pred_(actual_, expected_))
      return test_status::passed;
    else
...
static auto are_equal(A&& actual, E&& expect) 
{
  return detail_::AreEqual<A, E>{std::forward<A>(actual), std::forward<E>(expect)};
}

template <class Range>
static auto range_equal(Range&& range, std::initializer_list<typename std::decay_t<Range>::value_type> expected)
...
static auto permutation_execute(F&& target, Require&& req, Samples&&... samples)
...
static detail_::AssertBool<true> is_true(bool actual) { return {actual}; }

static detail_::AssertBool<false> is_false(bool actual) { return {actual}; }
...
template <class A, class E>
static detail_::AssertRelational<A, E, std::less<>>
less(A const& a, E const& e)
{
  return {a, e};
}

template <class A, class E>
static detail_::AssertRelational<A, E, std::less_equal<>>
less_equal(A const& a, E const& e)
{
  return {a, e};
}

template <class A, class E>
static detail_::AssertRelational<A, E, std::greater<>>
greater(A const& a, E const& e)
{
  return {a, e};
}

template <class A, class E>
static detail_::AssertRelational<A, E, std::greater_equal<>>
greater_equal(A const& a, E const& e)
{
  return {a, e};
}
