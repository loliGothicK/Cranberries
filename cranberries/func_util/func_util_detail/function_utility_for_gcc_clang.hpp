...
namespace cranberries_magic{
  ...
  constexpr decltype(auto) make_tuple_from_impl(Range&& range, std::index_sequence<I...>){
    return std::forward_as_tuple((range[I])...);
  }
}
...
namespace cranberries_magic {
  ...
  struct chunk_each_proxy {
    ...
    std::enable_if_t<disjunction_v<
      std::is_void<
        decltype( invoke<I>(std::declval<F>(), std::declval<std::tuple<Args...>>(), std::make_index_sequence<N>{}))
      >...
    >>
    apply(F&& f, std::index_sequence<I...>) const
      noexcept(conjunction_v<bool_constant<noexcept(invoke<I>(std::forward<F>(f), tup, std::make_index_sequence<N>{}))>...>)
    {
      using swallow = std::initializer_list<int>;
      (void)swallow {
  ...
  struct permutation_each_proxy {
    ...
    apply(F&& f, swallow_indices<swallow_t<I...>,swallow_t<Loop...>>) {
      using swallow = std::initializer_list<size_t>;
        (void)swallow {(void( f(arr[I]...) ), cranberries::next_partial_permutation( begin(arr), begin(arr)+R, end(arr) ), Loop)...};
    }
  ...
  struct combination_each_proxy {
    ...
    {
      using swallow = std::initializer_list<size_t>;
        (void)swallow {(void( f(arr[I]...) ), cranberries::next_combination( begin(arr), begin(arr)+R, end(arr) ), Loop)...};
    }
...
{
  return proxy.apply(std::forward<F>(f), make_swallow_indices<R,permutations(N,R)>{});
}

template < class F, class T, size_t N, size_t R >
constexpr auto operator |= (F&& f, cranberries_magic::combination_each_proxy<N, R, T> proxy)
{
  return proxy.apply(std::forward<F>(f), make_swallow_indices<R,combinations(N,R)>{});
}
