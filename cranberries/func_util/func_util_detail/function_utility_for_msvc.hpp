...
constexpr decltype(auto) make_tuple_from_impl(Range &&range,
...
constexpr decltype(auto) make_tuple_from(Range &&range) {
  return cranberries_magic::make_tuple_from_impl(std::forward<Range>(range),
                                                 std::make_index_sequence<N>{});
}

template <class F, class A, enabler_t<!enable_get_v<A>> = nullptr>
constexpr auto operator<<=(F &&f,
                           A &&a) noexcept(noexcept(f(std::forward<A>(a))))
    -> decltype(f(std::forward<A>(a))) {
  return f(std::forward<A>(a));
}

template <class F, class A, enabler_t<enable_get_v<A>> = nullptr>
constexpr auto operator<<=(F &&f, A &&a) noexcept(
...
constexpr std::enable_if_t<
    pack_disjunction_v<pack_t2v_transform_t<
        composition_<std::is_void, bind_2nd<bind_friendly_result_of,
                                            F>::template expr>::template pred,
        std::decay_t<Tuple>>>,
    void>
each_(F &&f, Tuple &&tup, swallow_t<I...>) noexcept(
    pack_conjunction_v<pack_t2v_transform_t<
        bind_2nd<bind_friendly_nothrow_callable, F>::template expr,
        std::decay_t<Tuple>>>) {
...
constexpr std::enable_if_t<
    pack_conjunction_v<pack_t2v_transform_t<
        composition_<
            negation, std::is_void,
            bind_2nd<bind_friendly_result_of, F>::template expr>::template pred,
        std::decay_t<Tuple>>>,
    pack_t2t_transform_t<bind_2nd<bind_friendly_result_of, F>::template expr,
                         std::decay_t<Tuple>>>
...
class random_sample_chunk_each_proxy {
 ...
 public:
  ...
  constexpr std::enable_if_t<std::is_void<::cranberries::invoke_result_t<
  ...
  template <class F, size_t... I, size_t... Loop>
  constexpr std::enable_if_t<
      !std::is_void<::cranberries::invoke_result_t<
          F, std::tuple_element_t<I, std::array<T, ElemSize>>...>>::value,
      generate_tuple_t<
          ::cranberries::invoke_result_t<
              F, std::tuple_element_t<I, std::array<T, ElemSize>>...>,
          Times>>
  apply(F &&f, std::index_sequence<I...>, std::index_sequence<Loop...>) {
...
struct permutation_each_proxy {
  ...
  apply(F &&f, swallow_t<I...>, swallow_t<Loop...>) {
    (void)SwallowsNest{(void(f(arr[I]...)),
...
constexpr auto operator|=(
  ...
  return proxy.apply(std::forward<F>(f), make_swallow_t<R>{},
...
struct permutation {
  ...
  static constexpr cranberries_magic::permutation_each_proxy<
      sizeof...(Args) + 1, (R == 0 ? sizeof...(Args) : R), std::decay_t<Head>>
