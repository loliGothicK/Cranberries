...
template <typename T>
CRANBERRIES_CXX11_CONSTEXPR typename std::remove_reference<T>::type &&move(
    T &&t) noexcept {
...
template <typename T>
CRANBERRIES_CXX11_CONSTEXPR T &&forward(
...
struct other_optional_copy_convertible
    : public std::integral_constant<
          bool, other_optional_copy_convertible_impl<T, U>::value> {};

template <typename T, typename U>
struct other_optional_move_convertible
    : public std::integral_constant<
          bool, other_optional_move_convertible_impl<T, U>::value> {};
...
struct other_optional_assignable
    : public std::integral_constant<
          bool, other_optional_assignable_impl<T, U>::value> {};
...
template <typename T, class optional_t>
class optional_impl<T, optional_t,
                    typename std::enable_if<!(
...
class optional_value_holder<
    T, typename std::enable_if<(std::is_class<T>::value ||
                                std::is_enum<T>::value)>::type> : public T {
 ...
 public:
  ...
  explicit CRANBERRIES_CXX11_CONSTEXPR optional_value_holder(std::true_type,
                                                             Args &&... args)
      : T(std::forward<Args>(args)...) {}
  template <typename U, typename... Args>
  explicit CRANBERRIES_CXX11_CONSTEXPR optional_value_holder(
      std::true_type, std::initializer_list<U> ilist, Args &&... args)
  ...
  CRANBERRIES_CXX14_CONSTEXPR T *get_addr() { return static_cast<T *>(this); }

  CRANBERRIES_CXX11_CONSTEXPR const T &get_ref() const & {
    return static_cast<const T &>(*this);
  }
  CRANBERRIES_CXX14_CONSTEXPR T &get_ref() & { return static_cast<T &>(*this); }
  CRANBERRIES_CXX11_CONSTEXPR const T &&get_ref() const && {
    return static_cast<const T &&>(*this);
  }
...
class optional_value_holder<
 ...
 public:
  ...
  CRANBERRIES_CXX11_CONSTEXPR const T *get_addr() const { return &value; }
  CRANBERRIES_CXX14_CONSTEXPR T *get_addr() { return &value; }

  CRANBERRIES_CXX11_CONSTEXPR const T &get_ref() const & {
    return static_cast<const T &>(value);
  }
  CRANBERRIES_CXX14_CONSTEXPR T &get_ref() & { return static_cast<T &>(value); }
  CRANBERRIES_CXX11_CONSTEXPR const T &&get_ref() const && {
    return static_cast<const T &&>(value);
  }
  CRANBERRIES_CXX14_CONSTEXPR T &&get_ref() && {
...
template <typename T>
class optional : private _detail::optional_impl<T, optional<T>> {
 private:
  ...
  union storage_t {
    ...
    explicit CRANBERRIES_CXX11_CONSTEXPR storage_t(std::false_type) : empty() {}
    // initialize with value
    template <typename... Args>
    explicit CRANBERRIES_CXX11_CONSTEXPR storage_t(std::true_type,
                                                   Args &&... args)
        : holder(std::true_type{}, std::forward<Args>(args)...) {}
    template <typename U, typename... Args>
    explicit CRANBERRIES_CXX11_CONSTEXPR storage_t(
        std::true_type, std::initializer_list<U> ilist, Args &&... args)
 ...
 private:
  ...
  CRANBERRIES_CXX14_CONSTEXPR void *getptr_impl() { return &storage; }
 ...
 public:
  ...
  template <
      typename Dummy = T,
      typename std::enable_if<(sizeof(Dummy), std::is_copy_constructible<
                                                  T>::value)>::type * = nullptr,
      typename std::enable_if<!(
          sizeof(Dummy), std::is_trivially_copy_constructible<T>::value)>::type
          * = nullptr>
  optional(const optional &_rhs) : storage(std::false_type{}), hasvalue(false) {
    if (_rhs.hasvalue) {
  ...
  CRANBERRIES_CXX11_CONSTEXPR optional(const optional &_rhs)
      // trivially-copyables can finish their copy by directly memmove their
      // underlying storage active member of union cannot be changed in
      // constexpr function
      : storage(_rhs.storage), hasvalue(_rhs.hasvalue) {}

  template <
      typename Dummy = T,
      typename std::enable_if<(sizeof(Dummy), std::is_move_constructible<
                                                  T>::value)>::type * = nullptr,
      typename std::enable_if<!(
          sizeof(Dummy), std::is_trivially_move_constructible<T>::value)>::type
          * = nullptr>
  optional(optional &&_rhs) noexcept(
      std::is_nothrow_move_constructible<T>::value)
      : storage(std::false_type{}), hasvalue(false) {
    if (_rhs.hasvalue) {
  ...
  template <
      ...
      typename std::enable_if<_detail::other_optional_copy_convertible<
          T, U>::value>::type * = nullptr,
      typename std::enable_if<std::is_convertible<const U &, T>::value>::type
          * = nullptr>
  optional(const optional<U> &_rhs)
      : storage(std::false_type{}), hasvalue(false) {
    if (_rhs.hasvalue) {
      new (getptr_impl())
          holder_t(std::true_type{}, _rhs.storage.holder.get_ref());
    }
  }
  template <
      typename U,
  ...
  template <
      typename U,
      typename std::enable_if<!(std::is_same<T, U>::value)>::type * = nullptr,
      typename std::enable_if<_detail::other_optional_move_convertible<
          T, U>::value>::type * = nullptr,
      typename std::enable_if<std::is_convertible<U &&, T>::value>::type * =
          nullptr>
  optional(optional<U> &&_rhs) : storage(std::false_type{}), hasvalue(false) {
    if (_rhs.hasvalue) {
      new (getptr_impl())
          holder_t(std::true_type{}, std::move(_rhs.storage.holder).get_ref());
    }
  }
  template <
      typename U,
  ...
  template <typename... Args, typename std::enable_if<std::is_constructible<
                                  T, Args...>::value>::type * = nullptr>
  CRANBERRIES_CXX11_CONSTEXPR explicit optional(in_place_t, Args &&... args)
      : storage(std::true_type{}, std::forward<Args>(args)...),
        hasvalue(true) {}

  template <
      typename U, typename... Args,
      typename std::enable_if<std::is_constructible<
          T, std::initializer_list<U> &, Args &&...>::value>::type * = nullptr>
  CRANBERRIES_CXX11_CONSTEXPR explicit optional(in_place_t,
                                                std::initializer_list<U> ilist,
                                                Args &&... args)
      : storage(std::true_type{}, ilist, std::forward<Args>(args)...),
        hasvalue(true) {}
  ...
  template <
      ...
      typename std::enable_if<std::is_constructible<T, U &&>::value>::type * =
          nullptr,
      typename std::enable_if<!(
          std::is_same<typename std::decay<U>::type, in_place_t>::value)>::type
          * = nullptr,
      typename std::enable_if<!(
  ...
  template <
      ...
      typename std::enable_if<(sizeof(Dummy), std::is_copy_constructible<
                                                  T>::value)>::type * = nullptr,
      typename std::enable_if<
          (sizeof(Dummy), std::is_copy_assignable<T>::value)>::type * = nullptr>
  ...
  template <
      ...
      typename std::enable_if<(sizeof(Dummy), std::is_move_constructible<
                                                  T>::value)>::type * = nullptr,
      typename std::enable_if<
          (sizeof(Dummy), std::is_move_assignable<T>::value)>::type * = nullptr>
  optional &operator=(optional &&_rhs) noexcept(
  ...
  template <
      typename U = T,
      typename std::enable_if<!(
          std::is_same<typename std::decay<T>::type, optional<T>>::value)>::type
          * = nullptr,
      typename std::enable_if<std::is_constructible<T, U>::value>::type * =
          nullptr,
  ...
  template <
      typename U,
      // to avoid collision with copy assignment operator that was intendedly
      // made template
      typename std::enable_if<!(std::is_same<T, U>::value)>::type * = nullptr,
      typename std::enable_if<
          _detail::other_optional_assignable<T, U>::value>::type * = nullptr,
      typename std::enable_if<std::is_constructible<T, const U &>::value>::type
          * = nullptr,
      typename std::enable_if<std::is_assignable<T &, const U &>::value>::type
          * = nullptr>
  ...
  optional &operator=(optional<U> &&_rhs) {
    ...
    assert(_rhs.hasvalue);
    if (hasvalue) {
      storage.holder.assign(
          std::move(std::move(_rhs.storage.holder).get_ref()));
    } else {
      new (getptr_impl())
          holder_t(std::move(std::move(_rhs.storage.holder).get_ref()));
      hasvalue = true;
    }
    return *this;
  }
  ...
  // ====================== operator->/operator* ======================
  CRANBERRIES_CXX11_CONSTEXPR const T *operator->() const { return getptr(); }
  CRANBERRIES_CXX14_CONSTEXPR T *operator->() { return getptr(); }

  CRANBERRIES_CXX11_CONSTEXPR const T &operator*() const & {
    return get_value();
  }
  CRANBERRIES_CXX14_CONSTEXPR T &operator*() & { return get_value(); }
  CRANBERRIES_CXX11_CONSTEXPR const T &&operator*() const && {
  ...
  // ====================== value_or ======================
  // the standard only requires the program is ill-formed if the enable_if cond
  // is not met. anyway...
  template <typename U,
  ...
  CRANBERRIES_CXX11_CONSTEXPR T value_or(U &&default_value) const & {
    return hasvalue ? storage.holder.get_ref()
  ...
  CRANBERRIES_CXX14_CONSTEXPR T value_or(U &&default_value) && {
    return hasvalue ? std::move(storage.holder).get_ref()
                    : static_cast<T>(std::forward<U>(default_value));
  }
  ...
  template <typename F,
            enabler_t<conjunction_v<
                ...
                ::cranberries::is_callable<F(), T>>> = nullptr>
  ...
  CRANBERRIES_CXX11_CONSTEXPR auto map(F &&f) const
    ...
    return hasvalue ? result_opt{f(storage.holder.get_ref())}
                    : result_opt{nullopt};
  }
  ...
  template <typename F, typename U,
            enabler_t<std::is_convertible<
                decltype(f(std::declval<const T &>())), U>::value> = nullptr>
  ...
  template <typename F, typename DefaultFn,
            enabler_t<std::is_convertible<
                ...
                decltype(std::declval<DefaultFn>()())>::value> = nullptr>
  CRANBERRIES_CXX11_CONSTEXPR auto map_or_else(F &&f,
                                               DefaultFn &&default_fn) const
      -> optional<decltype(f(std::declval<const T &>()))> {
    using result_opt = optional<decltype(f(std::declval<const T &>()))>;
    return hasvalue ? result_opt{f(storage.holder.get_ref())}
                    : result_opt{default_fn()};
  }
  ...
  // ====================== and_then ======================
  template <typename F,
            enabler_t<::cranberries::is_callable_v<F(T), T>> = nullptr>
  ...
  // ====================== or_else ======================
  template <typename F,
            enabler_t<std::is_same<remove_cvr_t<decltype(std::declval<F>()())>,
  ...
  // ====================== swap ======================
  void swap(optional &_rhs) noexcept(
      std::is_nothrow_move_constructible<T>::value
          && ::cranberries::is_nothrow_swappable<T>::value) {
    using std::swap;
    if ((!hasvalue) && (!(_rhs.hasvalue))) {
      return;
    }
    ...
    new (un->getptr_impl())
        holder_t(std::move(std::move(in->storage.holder).get_ref()));
    in->destroy();
    un->hasvalue = true;
    in->hasvalue = false;
    return;
  ...
  T &emplace(std::initializer_list<U> ilist, Args &&... args) {
    ...
    new (getptr_impl())
        holder_t(std::true_type{}, ilist, std::forward<Args>(args)...);
    hasvalue = true;
    return static_cast<optional &>(*this).get_value();
  }

 private:
  // actually, it might not matter whether this method has SFINAE restriction on
  // it or not.
  template <typename Dummy = T,
            typename std::enable_if<!(
                sizeof(Dummy), std::is_trivially_destructible<T>::value)>::type
                * = nullptr>
...
// ====================== operator==, !=, <, <=, >, >= ======================
template <typename T, typename U>
CRANBERRIES_CXX11_CONSTEXPR bool operator==(const optional<T> &_lhs,
                                            const optional<U> &_rhs) {
  return bool(_lhs) != bool(_rhs)
             ? false
             : (bool(_lhs) == false ? true : *_lhs == *_rhs);
}
template <typename T, typename U>
CRANBERRIES_CXX11_CONSTEXPR bool operator!=(const optional<T> &_lhs,
                                            const optional<U> &_rhs) {
  return bool(_lhs) != bool(_rhs)
             ? true
             : (bool(_lhs) == false ? false : *_lhs != *_rhs);
}
template <typename T, typename U>
CRANBERRIES_CXX11_CONSTEXPR bool operator<(const optional<T> &_lhs,
                                           const optional<U> &_rhs) {
  return bool(_rhs) == false ? false
                             : (bool(_lhs) == false ? true : *_lhs < *_rhs);
}
template <typename T, typename U>
CRANBERRIES_CXX11_CONSTEXPR bool operator<=(const optional<T> &_lhs,
                                            const optional<U> &_rhs) {
  return bool(_lhs) == false ? true
                             : (bool(_rhs) == false ? false : *_lhs <= *_rhs);
}
template <typename T, typename U>
CRANBERRIES_CXX11_CONSTEXPR bool operator>(const optional<T> &_lhs,
                                           const optional<U> &_rhs) {
  return bool(_lhs) == false ? false
                             : (bool(_rhs) == false ? true : *_lhs > *_rhs);
}
template <typename T, typename U>
CRANBERRIES_CXX11_CONSTEXPR bool operator>=(const optional<T> &_lhs,
                                            const optional<U> &_rhs) {
  return bool(_rhs) == false ? true
                             : (bool(_lhs) == false ? false : *_lhs >= *_rhs);
}

template <typename T>
CRANBERRIES_CXX11_CONSTEXPR bool operator==(const optional<T> &opt,
                                            nullopt_t) noexcept {
  return !opt;
}
template <typename T>
CRANBERRIES_CXX11_CONSTEXPR bool operator==(nullopt_t,
...
template <typename T, typename U>
CRANBERRIES_CXX11_CONSTEXPR bool operator==(const U &value,
...
// ====================== make_optional ======================
template <typename T>
CRANBERRIES_CXX11_CONSTEXPR optional<typename std::decay<T>::type>
...
CRANBERRIES_CXX11_CONSTEXPR optional<T> make_optional(
...
template <
    ...
    typename std::enable_if<std::is_move_constructible<
        T>::value && ::cranberries::is_swappable<T>::value>::type * = nullptr>
void swap(optional<T> &_lhs,
          optional<T> &_rhs) noexcept(noexcept(_lhs.swap(_rhs))) {
  _lhs.swap(_rhs);
}
...
struct has_hash_invoke_operator_helper {
  ...
  static auto test(int) -> typename std::is_same<
      decltype(std::declval<T>().operator()(
          std::declval<typename hash_argument_type<T>::type>())),
      std::size_t>::type;
  template <typename>
...
struct has_hash_invoke_operator_checker {
  constexpr static bool value =
      decltype(has_hash_invoke_operator_helper::test<T>(0))::value;
};

// if disabled hash reqiurement are not met, it does not matter if we mess
// things up more
template <class>
...
struct is_probably_hash_checker<std::hash<T>>
    : public ::cranberries::conjunctional<
          has_hash_invoke_operator_checker, std::is_default_constructible,
          std::is_copy_constructible, std::is_move_constructible,
          std::is_copy_assignable, std::is_move_assignable,
