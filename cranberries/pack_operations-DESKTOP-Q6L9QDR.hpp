...
namespace cranberries_magic {
	...
	template < size_t, size_t, class = std::index_sequence<> >
	struct make_index_range_impl;

	template < size_t First, size_t Last, size_t... Indices >
	struct make_index_range_impl<First, Last, std::index_sequence<Indices...>>
		: std::conditional_t<First!=Last,
			make_index_range_impl<First+1, Last, std::index_sequence<Indices..., First>>,
			nested_type_class<std::index_sequence<Indices..., First>>
		>
	{};

	template < size_t First, size_t Last >
	using make_index_range = typename make_index_range_impl<First, Last>::type;
