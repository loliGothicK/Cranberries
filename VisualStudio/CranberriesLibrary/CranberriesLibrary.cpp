...
int main() {
  ...
  UnitTestContainer{"Unit Test Example"}
    ...
    << assert::are_equal([] { std::this_thread::sleep_for(std::chrono::seconds(1)); return 2; }, 2)
    << assert::permutation_execute<2>([](auto a, auto b) { return a + b; }, assert::test_skip, 1, 2, 3, 4, 5)
    << assert::are_equal([] { return 100; }, 100)
    << assert::range_equal([] { return std::array<int, 4>{ {1, 2, 3, 4}}; }, 1, 2, 3)
